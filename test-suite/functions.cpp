...
TEST_CASE( "Functions_GammaFunction", "[Functions]" ) {
    ...
    Real expected = 0.0;
    Real calculated = std::lgamma(1);
    if (std::fabs(calculated) > 1.0e-15)
        FAIL_CHECK("GammaFunction(1)\n"
                    << std::setprecision(16) << QL_SCIENTIFIC
                    << "    calculated: " << calculated << "\n"
                    << "    expected:   " << expected);
    ...
    for (Size i=2; i<9000; i++) {
        ...
        calculated = std::lgamma(static_cast<Real>(i+1));
        if (std::fabs(calculated-expected)/expected > 1.0e-9)
            FAIL_CHECK("GammaFunction(" << i << ")\n"
                        << std::setprecision(16) << QL_SCIENTIFIC
                        << "    calculated: " << calculated << "\n"
                        << "    expected:   " << expected << "\n"
                        << "    rel. error: "
...
TEST_CASE( "Functions_GammaValues", "[Functions]" ) {
    ...
    for (Size i=0; i < LENGTH(tasks); ++i) {
        ...
        const Real calculated = std::tgamma(x);
        const Real tol = tasks[i][2] * QL_EPSILON*std::fabs(expected);

        if (std::fabs(calculated - expected) > tol) {
            FAIL_CHECK("GammaFunction(" << x << ")\n"
                        << std::setprecision(16) << QL_SCIENTIFIC
                        << "    calculated: " << calculated << "\n"
                        << "    expected:   " << expected << "\n"
                        << "    rel. error: "
...
TEST_CASE( "Functions_ModifiedBesselFunctions", "[Functions]" ) {
    ...
    for (Size i=0; i < LENGTH(r); ++i) {
        ...
        if (std::fabs(expected_i - calculated_i) > tol_i) {
            FAIL_CHECK("failed to reproduce modified Bessel "
...
TEST_CASE( "Functions_WeightedModifiedBesselFunctions", "[Functions]" ) {
    ...
    while (nu <= 5.0) {
        ...
        while (x <= 15.0) {
            ...
            if (std::abs(expected_i - calculated_i) > tol_i) {
                FAIL_CHECK("failed to verify exponentially weighted"
