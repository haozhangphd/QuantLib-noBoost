...
namespace {
    ...
    struct CommonVars {
        ...
        // utilities
        std::shared_ptr<VanillaSwap> makeSwap(Integer length, Rate fixedRate,
        ...
        CommonVars() {
            ...
            index = std::make_shared<Euribor>(Period(floatingFrequency), termStructure);
    ...
    for (Size i=0; i<LENGTH(lengths); i++) {
        for (Size j=0; j<LENGTH(spreads); j++) {
            ...
            if (std::fabs(swap->NPV()) > 1.0e-10) {
                FAIL_CHECK("recalculating with implied rate:\n"
                            << std::setprecision(2)
...
TEST_CASE("Swap_FairSpread", "[Swap]") {
    ...
    for (Size i=0; i<LENGTH(lengths); i++) {
        for (Size j=0; j<LENGTH(rates); j++) {
            ...
            if (std::fabs(swap->NPV()) > 1.0e-10) {
                FAIL_CHECK("recalculating with implied spread:\n"
                            << std::setprecision(2)
                            << "    length: " << lengths[i] << " years\n"
    ...
    for (Size i=0; i<LENGTH(lengths); i++) {
        for (Size j=0; j<LENGTH(spreads); j++) {
            ...
            for (Size k=0; k<LENGTH(rates); k++) {
                ...
                swap_values.emplace_back(swap->NPV());
            }
...
TEST_CASE("Swap_InArrears", "[Swap]") {
    ...
    */
    ...
    std::shared_ptr<IborIndex> index = std::make_shared<IborIndex>("dummy", 1*Years, 0,
    ...
    Handle<OptionletVolatilityStructure> vol(
        std::make_shared<ConstantOptionletVolatility>(vars.today, NullCalendar(), Following,
    ...
    if (std::fabs(swap.NPV()-storedValue) > tolerance)
        FAIL_CHECK("Wrong NPV calculation:\n"
...
TEST_CASE("Swap_CachedValue", "[Swap]") {
    ...
    if (std::fabs(swap->NPV()-cachedNPV) > 1.0e-11)
        FAIL_CHECK("failed to reproduce cached swap value:\n"
