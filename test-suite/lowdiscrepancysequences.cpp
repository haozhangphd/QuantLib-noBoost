...
TEST_CASE("LowDiscrepancy_PolynomialsModuloTwo", "[LowDiscrepancy]") {
    ...
    while (n<PPMT_MAX_DIM || polynomial!=-1) {
        ...
        if (polynomial==-1) {
            --n;
            if (j!=jj[i]) {
                FAIL_CHECK("Only " << j << " polynomials in degree " << i+1
                            << " instead of " << jj[i]);
            }
        }
...
namespace
{
                                   ...
                                   const std::string& nameString)
    {
        ...
        INFO("Testing randomized lattice sequences (" << nameString
        ...
        for (Size i=0; i < maxDim; ++i)
            if (fabs(errorInSds[i] ) > tolerance)
                FAIL_CHECK("Lattice generator" << nameString <<" returns  a mean of " <<
...
TEST_CASE("LowDiscrepancy_Sobol", "[LowDiscrepancy]") {
    ...
    for (i=0; i<points; i++) {
        ...
        if (point.size()!=dimensionality) {
            FAIL_CHECK("Sobol sequence generator returns "
    ...
    for (Integer j=1; j<5; j++) { // five cycle
        ...
        for (i=0; i<dimensionality; i++) {
            ...
            if (error > tolerance) {
                FAIL_CHECK(io::ordinal(i+1) << " dimension: "
                            << QL_FIXED
                            << "mean (" << mean[i]
    ...
    for (i=0; i<points; i++) {
        ...
        if (error > tolerance) {
            FAIL_CHECK(io::ordinal(i+1) << " draw ("
                        << QL_FIXED << point[0]
...
TEST_CASE("LowDiscrepancy_Faure", "[LowDiscrepancy]") {
    ...
    for (i=0; i<points; i++) {
        point = rsg.nextSequence().value;
        if (point.size()!=dimensionality) {
            FAIL_CHECK("Faure sequence generator returns "
                        " a sequence of wrong dimensionality: " << point.size()
                        << " instead of  " << dimensionality);
        }
    }
    ...
    for (i=0; i<points; i++) {
        ...
        Real error = std::fabs(point[0]-vanderCorputSequenceModuloTwo[i]);
        if (error > tolerance) {
            FAIL_CHECK(io::ordinal(i+1) << " draw, dimension 1 ("
                        << QL_FIXED << point[0]
    ...
    for (i=0; i<points; i++) {
        ...
        if (error > tolerance) {
            FAIL_CHECK(io::ordinal(i+1) << " draw, dimension 2 ("
                        << QL_FIXED << point[1]
                        << ") in 3-D Faure sequence should have been "
    ...
    for (i=0; i<points; i++) {
        ...
        if (error > tolerance) {
            FAIL_CHECK(io::ordinal(i+1) << " draw, dimension 3 ("
                        << QL_FIXED << point[2]
                        << ") in 3-D Faure sequence should have been "
...
TEST_CASE("LowDiscrepancy_Halton", "[LowDiscrepancy]") {
    ...
    for (i=0; i<points; i++) {
        point = rsg.nextSequence().value;
        if (point.size()!=dimensionality) {
            FAIL_CHECK("Halton sequence generator returns "
                        " a sequence of wrong dimensionality: " << point.size()
                        << " instead of  " << dimensionality);
        }
    }
    ...
    for (i=0; i<points; i++) {
        ...
        Real error = std::fabs(point[0]-vanderCorputSequenceModuloTwo[i]);
        if (error > tolerance) {
            FAIL_CHECK("First component of " << io::ordinal(i+1)
        ...
        if (error > tolerance) {
            FAIL_CHECK("Second component of " << io::ordinal(i+1)
    ...
    for (j=1; j<5; j++) { // five cycle
        ...
        if (error > tolerance) {
            FAIL_CHECK("First dimension mean (" << QL_FIXED << mean[0]
                        << ") at the end of the " << io::ordinal(j+1)
    ...
    for (j=1; j<3; j++) { // three cycle
        ...
        if (error > tolerance) {
            FAIL_CHECK("Second dimension mean (" << QL_FIXED << mean[1]
                        << ") at the end of the " << io::ordinal(j+1)
                        << " cycle in Halton sequence is not " << 0.5
                        << QL_SCIENTIFIC
                        << " (error = " << error << ")");
        ...
        for (Integer i = 0; i<8; i++) {
            ...
            for (j=jMin; j<jMin+sampleLoops; j++) {
                ...
                if (std::fabs(discr-discrepancy[i][j-jMin])>tolerance*discr) {
                    FAIL_CHECK(generatorFactory.name()
