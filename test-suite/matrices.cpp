	...
	Real minHolder = QL_MAX_REAL;
        ...
        for (Size i=0; i<N; i++) {
            ...
            if (eigenValues[i] >= minHolder) {
                FAIL("Eigenvalues not ordered: " << eigenValues);
            } else
...
TEST_CASE( "Matrices_Sqrt", "[Matrices]" ) {
    ...
    if (error>tolerance) {
        FAIL("Matrix square root calculation failed\n"
                   << "original matrix:\n" << M1
                   << "pseudoSqrt:\n" << m
...
TEST_CASE( "Matrices_HighamSqrt", "[Matrices]" ) {
    ...
    if (error>tolerance) {
        FAIL("Higham matrix correction failed\n"
                   << "original matrix:\n" << M5
                   << "pseudoSqrt:\n" << tempSqrt
                   << "should be:\n" << ansSqrt
                   << "\nerror:     " << error
...
TEST_CASE( "Matrices_QRDecomposition", "[Matrices]" ) {
    ...
    for (Size j = 0; j < LENGTH(testMatrices); j++) {
        ...
        if (norm(Q*R - A*P) > tol)
            FAIL("Q*R does not match matrix A*P (norm = "
                       << norm(Q*R-A*P) << ")");
        ...
        if (norm(Q*R - A) > tol)
            FAIL("Q*R does not match matrix A (norm = "
                       << norm(Q*R-A) << ")");
    }
}
...
TEST_CASE( "Matrices_QRSolve", "[Matrices]" ) {
    ...
    for (Size j = 0; j < LENGTH(testMatrices); j++) {
        ...
        for (Size k=0; k < 10; ++k) {
            ...
            if (A.columns() >= A.rows()) {
                if (norm(A*x - b) > tol)
                    FAIL("A*x does not match vector b (norm = "
                               << norm(A*x - b) << ")");
            }
            else {
                ...
                if (norm(xr-x) > tol) {
                    FAIL("least square solution does not match (norm = "
                               << norm(x - xr) << ")");
...
TEST_CASE( "Matrices_OrthogonalProjection", "[Matrices]" ) {
    ...
    if (numberFailures > 0 || failuresTwo >0)
        FAIL("OrthogonalProjections test failed with " << numberFailures << " failures  of orthogonality and "
...
TEST_CASE( "Matrices_CholeskyDecomposition", "[Matrices]" ) {
    ...
    for(Size i=0;i<11;++i) {
        for(Size j=0;j<11;++j) {
            if(std::isnan(m2[i][j])) {
                FAIL("Faield to verify Cholesky decomposition at (i,j)=("
...
TEST_CASE( "Matrices_MoorePenroseInverse", "[Matrices]" ) {
    ...
    for (Size i = 0; i < 6; ++i) {
        if (std::abs(x[i] - cached[i]) > tol) {
            FAIL("Failed to verify minimal norm solution obtained from "
