...
namespace {
    ...
    void checkBivariate(const char* tag) {
        ...
        for (Size i=0; i<LENGTH(values); i++) {
            ...
            if (std::fabs(value-values[i].result) >= tolerance) {
                FAIL_CHECK(tag << " bivariate cumulative distribution\n"
                            << "    case: " << i+1 << "\n"
                            << QL_FIXED
                            << "    a:    " << values[i].a << "\n"
                            << "    b:    " << values[i].b << "\n"
    ...
    void checkBivariateTail(const char* tag, Real tolerance) {
        ...
        for (int i = 0; i<10;i++) {
            ...
            Real cdf1 = bvn(x,y);
            if (cdf0 > cdf1) {
                FAIL_CHECK(tag << " cdf must be decreasing in the tails\n"
                            << QL_SCIENTIFIC
                            << "    cdf0: " << cdf0 << "\n"
                            << "    cdf1: " << cdf1 << "\n"
                            << "    x: " << x << "\n"
...
TEST_CASE("Distribution_Normal", "[Distribution]") {
    ...
    if (check != 0.0e0) {
        FAIL_CHECK("C++ inverse cumulative of the standard normal at 0.5 is "
                    << QL_SCIENTIFIC << check
    ...
    if (e > 1.0e-16) {
        FAIL_CHECK("norm of C++ NormalDistribution minus analytic Gaussian: "
                    << QL_SCIENTIFIC << e << "\n"
                    << "tolerance exceeded");
    }
    ...
    if (e > 1.0e-7) {
        FAIL_CHECK("norm of invCum . cum minus identity: "
                    << QL_SCIENTIFIC << e << "\n"
                    << "tolerance exceeded");
    }
    ...
    e = norm(diff.begin(), diff.end(), h);
    if (e > 1.0e-7) {
        FAIL_CHECK("norm of MaddokInvCum . cum minus identity: "
                    << QL_SCIENTIFIC << e << "\n"
                    << "tolerance exceeded");
    }
    ...
    e = norm(diff.begin(),diff.end(),h);
    if (e > 1.0e-16) {
        FAIL_CHECK("norm of C++ Normal.derivative minus analytic derivative: "
                    << QL_SCIENTIFIC << e << "\n"
                    << "tolerance exceeded");
    }
}
...
TEST_CASE("Distribution_BivariateCumulativeStudent", "[Distribution]") {
    ...
    for (Size i=0; i < LENGTH(cases); ++i) {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            FAIL_CHECK("Failed to reproduce CDF value:" <<
                        "\n    n:   " << cases[i].n <<
                        "\n    rho: " << cases[i].rho <<
                        "\n    x:   " << cases[i].x <<
                        "\n    y:   " << cases[i].y <<
...
TEST_CASE("Distribution_InvCDFviaStochasticCollocation", "[Distribution]") {
    ...
    const InverseCumulativeNonCentralChiSquare invCDF(k, lambda);
    ...
    for (Real x=-3.0; x < 3.0; x+=0.1) {
        ...
        if (std::fabs(calculated1 - calculated2) > 1e-6) {
            FAIL("Failed to reproduce equal stochastic collocation "
                       "inverse CDF" <<
                       "\n    x: " << x <<
        ...
        if (std::fabs(calculated2 - expected) > tol) {
            FAIL("Failed to reproduce invCDF with "
