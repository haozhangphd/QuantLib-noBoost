...
namespace {
    ...
    std::vector<Size> makeMeasure(const MarketModelMultiProduct& product,
        ...
        switch (measureType) {
          ...
          case Terminal:
            ...
            if (!isInTerminalMeasure(evolution, result)) {
                FAIL_CHECK("failure in verifying Terminal measure:\n"
                            << to_stream(result));
            }
            break;
          case MoneyMarket:
            ...
            if (!isInMoneyMarketMeasure(evolution, result)) {
                FAIL_CHECK("failure in verifying MoneyMarket measure:\n"
                            << to_stream(result));
            }
            break;
          case MoneyMarketPlus:
            ...
            if (!isInMoneyMarketPlusMeasure(evolution, result,
                                            measureOffset_)) {
                FAIL_CHECK("failure in verifying MoneyMarketPlus(" <<
        ...
        if (printReport_) {
            INFO("    " << measureTypeToString(measureType) << ": "
                               << to_stream(result));
        }
        return result;
    }
    ...
    std::shared_ptr<MarketModelEvolver> makeMarketModelEvolver(
        ...
        switch (evolverType) {
          case Pc:
            return std::make_shared<LogNormalCmSwapRatePc>(spanningForwards,
    ...
    void checkCMSAndSwaptions(
              ...
              const std::vector<std::shared_ptr<StrikedTypePayoff> >&
                                                              displacedPayoff,
              const std::shared_ptr<MarketModel>, // marketModel,
