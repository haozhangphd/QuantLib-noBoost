...
TEST_CASE("TimeSeries_Construction", "[TimeSeries]") {
    ...
    if (cur->first != Date(15, March, 2005)) {
        FAIL_CHECK("date does not match");
    }
    if (cur->second != 0.3) {
        FAIL_CHECK("value does not match");
    }

    ts[Date(15, March, 2005)] = 4.0;
    cur = ts.begin();
    if (cur->second != 4.0) {
        FAIL_CHECK("replaced value does not match" << cur->second << "\n");
    }

    ts[Date(15, March, 2005)] = 3.5;

    if (cur->second != 3.5) {
        FAIL_CHECK("set value operator not match" << cur->second << "\n");
    }
}
...
TEST_CASE("TimeSeries_IntervalPrice", "[TimeSeries]") {
    ...
    date.emplace_back(Date(25, March, 2005));
    date.emplace_back(Date(29, March, 2005));

    open.emplace_back(1.3);
    open.emplace_back(2.3);

    close.emplace_back(2.3);
    close.emplace_back(3.4);

    high.emplace_back(3.4);
    high.emplace_back(3.5);

    low.emplace_back(3.4);
    low.emplace_back(3.2);
...
namespace std {
    ...
    struct hash<Date> {
        size_t operator()(const Date& _Keyval) const {
            return std::hash<Date::serial_type>()(_Keyval.serialNumber());
        }
    };
...
TEST_CASE("TimeSeries_Iterators", "[TimeSeries]") {
    ...
    dates.emplace_back(Date(25, March, 2005));
    dates.emplace_back(Date(29, March, 2005));
    dates.emplace_back(Date(15, March, 2005));

    prices.emplace_back(25);
    prices.emplace_back(23);
    prices.emplace_back(20);
    ...
    // unordered container
    typedef TimeSeries<int, std::unordered_map<Date, int> >
                                                          TimeSeriesUnordered;
    ...
    if (ts.lastDate() != Date(29, March, 2005)) {
        FAIL_CHECK("lastDate does not match");
    }
}
