...
namespace {
    ...
    void setup() {
        ...
        yoyIndexUK = std::make_shared<YYUKRPIr>(true, yoyUK);
        yoyIndexEU = std::make_shared<YYEUHICPr>(true, yoyEU);
        ...
        std::shared_ptr<InterpolatedZeroCurve<Cubic>> euriborTS = std::make_shared<InterpolatedZeroCurve<Cubic>>(d, r,
                                                                                                                 Actual365Fixed());
        ...
        std::shared_ptr<InterpolatedZeroCurve<Cubic>> gbpLiborTS = std::make_shared<InterpolatedZeroCurve<Cubic>>(d, r,
                                                                                                                  Actual365Fixed());
        ...
        for (Size i = 0; i < LENGTH(yoyEUrates); i++) {
            ...
            d.emplace_back(dd);
            r.emplace_back(yoyEUrates[i]);
        }

        bool indexIsInterpolated = true;    // actually false for UKRPI but smooth surfaces are
        // better for finding intersections etc

        std::shared_ptr<InterpolatedYoYInflationCurve<Linear>> pYTSEU = std::make_shared<InterpolatedYoYInflationCurve<Linear>>(
                eval, TARGET(), Actual365Fixed(), Period(2, Months), Monthly, indexIsInterpolated, nominalGBP, d, r);
        yoyEU.linkTo(pYTSEU);
        ...
        fStrikesEU.clear();
        cfMaturitiesEU.clear();
        for (Size i = 0; i < ncStrikesEU; i++) cStrikesEU.emplace_back(capStrikesEU[i]);
        for (Size i = 0; i < nfStrikesEU; i++) fStrikesEU.emplace_back(floorStrikesEU[i]);
        for (Size i = 0; i < ncfMaturitiesEU; i++) cfMaturitiesEU.emplace_back(capMaturitiesEU[i]);
        std::shared_ptr < Matrix > tcPriceEU = std::make_shared<Matrix>(ncStrikesEU, ncfMaturitiesEU);
        std::shared_ptr < Matrix > tfPriceEU = std::make_shared<Matrix>(nfStrikesEU, ncfMaturitiesEU);
    ...
    void setupPriceSurface() {
        ...
        std::shared_ptr<InterpolatedYoYCapFloorTermPriceSurface<Bicubic, Cubic>> cfEUprices = std::make_shared<InterpolatedYoYCapFloorTermPriceSurface<Bicubic, Cubic>>(
                fixingDays, yyLag, yoyIndexEU, baseRate, n, dc, cal, bdc, cStrikesEU, fStrikesEU, cfMaturitiesEU,
                (*cPriceEU), (*fPriceEU));
...
TEST_CASE("InflationVol_YoYPriceSurfaceToVol", "[InflationVol]") {
    ...
    Handle<YoYOptionletVolatilitySurface> hVS(pVS,
                                              false); // pVS does NOT own whatever it points to later, hence the handle does not either
    std::shared_ptr < YoYInflationUnitDisplacedBlackCapFloorEngine >
    yoyPricerUD = std::make_shared<YoYInflationUnitDisplacedBlackCapFloorEngine>(yoyIndexEU, hVS); //hVS
    // N.B. the vol gets set in the stripper ... else no point!

    // cap stripper
    std::shared_ptr < YoYOptionletStripper > yoyOptionletStripper =
            std::make_shared<InterpolatedYoYOptionletStripper<Linear>>();
    ...
    std::shared_ptr<KInterpolatedYoYOptionletVolatilitySurface<Linear>> yoySurf =
            std::make_shared<KInterpolatedYoYOptionletVolatilitySurface<Linear>>(settlementDays,
