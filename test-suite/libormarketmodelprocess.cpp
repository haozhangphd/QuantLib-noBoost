    ...
    for (Integer daysOffset=0; daysOffset < 1825 /* 5 year*/; daysOffset+=8) {
        ...
        for (Size i=1; i < fixings.size()-1; ++i) {
            ...
            if ((ileft != i) || (iright != i+1) || (ii != i+1)) {
                FAIL_CHECK("Failed to next index resets");
            }
        }
...
TEST_CASE("LiborMarketModelProcess_LambdaBootstrapping", "[LiborMarketModelProcess]") {
    ...
    for (Size i=0; i<9; ++i) {
        ...
        if (std::fabs(calculated - expected) > tolerance)
            FAIL_CHECK("Failed to reproduce expected lambda values"
...
TEST_CASE("LiborMarketModelProcess_MonteCarloCapletPricing", "[LiborMarketModelProcess]") {
    ...
    for (Size k=0; k < process1->size(); ++k) {
        ...
        if (std::fabs(calculated1 - expected) > tolerance1) {
            FAIL_CHECK("Failed to reproduce expected caplet NPV"
