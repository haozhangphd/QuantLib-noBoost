...
*/
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        procs.emplace_back(stochProcess3);
...
void OneDAmericanValues(unsigned from, unsigned to) {

    INFO("Testing basket American options against 1-D case "
                       "from " << from << " to " << to-1 <<  "...");
    ...
    for (Size i=from; i<to; i++) {
        ...
        if (relError > oneDataValues[i].tol) {
            FAIL("expected value: " << oneDataValues[i].result << "\n"
...
TEST_CASE("BasketOption_OddSamples", "[BasketOption]") {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        if (relError > values[i].tol) {
            FAIL("expected value: " << values[i].result << "\n"
                       << "calculated:     " << calculated);
        }
    }
}
...
TEST_CASE("BasketOption_LocalVolatilitySpreadOption", "[BasketOption]") {
    ...
    if (std::fabs(expected - calculated) > tolerance) {
        FAIL_CHECK("Failed to reproduce expected local volatility price"
                    << "\n    calculated: " << calculated
                    << "\n    expected:   " << expected
                    << "\n    tolerance:  " << tolerance);
    }
...
TEST_CASE("BasketOption_2DPDEGreeks", "[BasketOption]") {
    ...
    if (std::fabs(expectedDelta - calculatedDelta) > tol) {
        FAIL("failed to reproduce delta with 2dim PDE"
