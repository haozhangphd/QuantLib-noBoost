            ...
            engine = std::make_shared<FdHestonDoubleBarrierEngine>(
                std::make_shared<HestonModel>(
                    std::make_shared<HestonProcess>(
                        ...
                        Handle<YieldTermStructure>(qTS),
                        Handle<Quote>(spot),
                        square(vol->value()), 1.0,
                        square(vol->value()), 0.001, 0.0)), 251, 76, 3);
...
TEST_CASE( "DoubleBarrierOption_VannaVolgaDoubleBarrierValues", "[DoubleBarrierOption]" ) {
    ...
    for (Size i=0; i<LENGTH(values); i++) {
        ...
        std::shared_ptr<StrikedTypePayoff> payoff =
            std::make_shared<PlainVanillaPayoff>(values[i].type,
                                                   values[i].strike);
        ...
        std::shared_ptr<PricingEngine> vannaVolgaEngine =
            std::make_shared<VannaVolgaDoubleBarrierEngine<WulinYongDoubleBarrierEngine> >(
                            volAtmQuote,
							vol25PutQuote,
							vol25CallQuote,
							Handle<Quote> (spot),
