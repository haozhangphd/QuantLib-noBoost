...
namespace {
    ...
    void check(const std::string& name) {
        ...
        if (s.samples() != LENGTH(data))
            FAIL(name << ": wrong number of samples\n"
...
#define TEST_INC_STAT(expr, expected)                                          \
    if (!close_enough(expr, expected))                                         \
        FAIL_CHECK(std::setprecision(16)                                      \
...
TEST_CASE("Statistics_IncrementalStatistics", "[Statistics]") {
    ...
    if (stat.samples() != 500000)
        FAIL_CHECK("stat.samples()  (" << stat.samples()
                    << ") can not be reproduced against cached result ("
                    << 500000 << ")");

    // The skewness and kurtosis values are very slightly different from
    // the values in unmodified QuantLib. These values are verified with
    // a multi-precision library (libquadmath).
    ...
    TEST_INC_STAT(stat.skewness(), -1.7360169326722651e-03);
    TEST_INC_STAT(stat.kurtosis(), -1.1990742562086139e+00);
    ...
    if(std::fabs(stat2.variance() - 1E-2 ) > tol)
        FAIL_CHECK("variance (" << stat2.variance()
