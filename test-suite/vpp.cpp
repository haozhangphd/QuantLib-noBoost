...
TEST_CASE("VPP_GemanRoncoroniProcess", "[VPP]") {
    ...
    */
    ...
    processes.emplace_back(eouProcess);
...
TEST_CASE("VPP_SimpleExtOUStorageEngine", "[VPP]") {
    ...
    if (std::fabs(expected - calculated) > 5e-2) {
        FAIL_CHECK("Failed to reproduce cached values" <<
...
TEST_CASE("VPP_KlugeExtOUSpreadOption", "[VPP]") {
    ...
    if (std::fabs(expectedMC - calculated) > 3*mcError) {
            FAIL_CHECK("Failed to reproduce referenc values"
                       << "\n    calculated:   " << calculated
...
TEST_CASE("VPP_VPPIntrinsicValue", "[VPP]") {
    ...
    for (Size i=0; i < LENGTH(efficiency); ++i) {
        ...
        if (std::fabs(expected[i] - calculated) > 1e-4) {
            FAIL_CHECK("Failed to reproduce reference values"
                       << "\n    calculated: " << calculated
                       << "\n    expected:   " << expected[i]);
    ...
    class PathSparkSpreadPrice : public FdmInnerValueCalculator {
      public:
        ...
        PathSparkSpreadPrice(
            Real heatRate,
            const MultiPathGenerator<PseudoRandom>::sample_type::value_type& path,
            const std::shared_ptr<Shape>& fuelShape,
            const std::shared_ptr<Shape>& powerShape)
        : heatRate_(heatRate),
          path_(path),
...
TEST_CASE("VPP_VPPPricing", "[VPP]") {
    ...
    for (Size i=0; i < nHours; ++i) {
        ...
        const Real gs = std::log(fuelPrice)-square(volatility_u)
                               /(4*kappa)*(1-std::exp(-2*kappa*t));
        (*fuelShape)[i] = Shape::value_type(t, gs);

        const Real powerPrice = powerPrices[i];
        const Real ps = std::log(powerPrice)-square(volatility_x)
    ...
    if (std::fabs(intrinsic - expectedIntrinsic) > 0.1) {
        FAIL_CHECK("Failed to reproduce intrinsic value"
    ...
    if (std::fabs(fdmPrice - expectedFdmPrice) > 0.1) {
       FAIL_CHECK("Failed to reproduce finite difference price"
                   << "\n    calculated: " << fdmPrice
    ...
    if (std::fabs(npvMC-expectedMC) > 3*errorMC) {
        FAIL_CHECK("Failed to reproduce Monte-Carlo price"
                   << "\n    calculated: " << npvMC
                   << "\n    error     ; " << errorMC
                   << "\n    expected  : " << expectedMC);
    }
    ...
    const Size dim = 1u;
    std::vector<std::function<Real(const Array&)> > v(
    ...
    if (std::fabs(npvMC-fdmPrice) > 3*errorMC) {
        FAIL_CHECK("Failed to reproduce Least Square Monte-Carlo price"
                   << "\n    calculated   : " << npvMC
                   << "\n    error        : " << errorMC
...
TEST_CASE("VPP_KlugeExtOUMatrixDecomposition", "[VPP]") {
    ...
    for (Size i=0; i < x.size(); ++i) {
        ...
        if (diffApply > tol && diffApply > std::fabs(applyExpected[i])*tol) {
            FAIL_CHECK("Failed to reproduce apply operation" <<
