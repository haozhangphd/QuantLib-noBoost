...
TEST_CASE("CreditRiskPlus_ReferenceValues", "[CreditRiskPlus]") {
    ...
    relativeDefaultVariance.emplace_back(0.75 * 0.75);
    relativeDefaultVariance.emplace_back(0.75 * 0.75);
    ...
    if ( std::fabs(cr.relativeDefaultVariance() - 0.65 * 0.65) > 0.001 )
        FAIL("failed to reproduce overall relative default variance ("
                   << cr.relativeDefaultVariance() << ", should be 0.4225)");

    if ( std::fabs(cr.lossQuantile(0.99) - 250) > 0.5 )
        FAIL("failed to reproduce overall 99 percentile ("
