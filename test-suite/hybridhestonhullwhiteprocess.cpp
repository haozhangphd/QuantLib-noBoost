...
namespace {
    ...
    std::shared_ptr<HestonProcess> makeHestonProcess(
        ...
        Handle<Quote> spot(
                std::shared_ptr<SimpleQuote>(new SimpleQuote(100)));
        ...
        return std::shared_ptr<HestonProcess>(new HestonProcess(
    ...
    std::shared_ptr<VanillaOption> makeVanillaOption(
        ...
        return std::shared_ptr<VanillaOption>(
...
TEST_CASE("HybridHestonHullWhiteProcess_HestonHullWhiteCalibration", "[HybridHestonHullWhiteProcess]") {
        ...
        for (Size j=0; j < LENGTH(strikes); ++j) {
            std::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(
                strikes[j] * rTS->discount(maturities[i]) >=
                    ...
                    : Option::Put,
                strikes[j]));
            RelinkableHandle<Quote> v(std::shared_ptr<Quote>(
    ...
    for (Size i=0; i < LENGTH(maturities); ++i) {
        ...
        for (Size j=0; j < LENGTH(strikes); ++j) {
            ...
            Handle<Quote> v(std::shared_ptr<Quote>(
