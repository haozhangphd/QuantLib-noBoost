    ...
    for (Size i=0; i<n; i++) {
        for (Size j=0; j<n; j++) {
            ...
            if (std::fabs(calculated-expected) > 1.0e-10)
                FAIL_CHECK("Salvaging correlation with spectral alg "
...
//    Matrix b = pseudoSqrt(badCorr, Hypersphere);
    ...
    for (Size i=0; i<n; i++) {
        for (Size j=0; j<n; j++) {
            ...
            calculated = calcCorr[i][j];
            if (std::fabs(calculated-expected) > 1.0e-10)
                FAIL_CHECK("SalvagingCorrelation with spectral alg "
                            << "cor[" << i << "][" << j << "]:\n"
                            << std::setprecision(10)
                            << "    calculated: " << calculated << "\n"
                            << "    expected:   " << expected);
...
TEST_CASE("Covariance_Covariance", "[Covariance]") {
    ...
    for (i=0; i<n; i++) {
        ...
        if (std::fabs(calculated-expected) > 1.0e-16) {
            FAIL_CHECK("CovarianceDecomposition "
                        << "standardDev[" << i << "]:\n"
