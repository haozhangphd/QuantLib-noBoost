    ...
    for (Size k=0; k<3; ++k) {
        ...
        for (i=0; i<v.size(); ++i) {
            if (m.standardErrors()[i] > tolerance) {
                FAIL_CHECK("Failed to reproduce linear regression coef."
...
TEST_CASE( "LinearLeastSquaresRegression_MultiDimRegression", "[LinearLeastSquaresRegression]" ) {
    ...
    for (Size i=0; i < dims; ++i) {
        v.emplace_back([i](const Array & a){return f(a, i);});
    }
    ...
    * Multiple linear regression/weighted regression, Boris Skorodumov */
    ...
    for (Size i=0; i < 2; ++i) {
        if (std::fabs(m.standardErrors()[i]-errorsExpected[i]) > tol) {
            FAIL_CHECK("Failed to reproduce linear regression standard errors"
