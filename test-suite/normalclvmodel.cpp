...
TEST_CASE("NormalCLVModel_Illustrative1DExample", "[NormalCLVModel]") {
    ...
    std::vector<Date> maturityDates{(today + Period(18, Days)),
    ...
    x {{ 1.070, 0.984, 0.903, 0.817 },{ 0.879, 0.668, 0.472, 0.261 },{ 0.528, 0.282, 0.052, -0.194 }};

    std::vector<std::vector<Real> >
    s {{ 1.104, 1.035, 0.969, 0.895 },{ 1.328, 1.122, 0.911, 0.668 }, { 1.657, 1.283, 0.854, 0.339 }};
    ...
    for (Size i = 0; i < maturities.size(); ++i) {
        ...
        for (Size j = 0; j < x.front().size(); ++j) {
            ...
            if (std::fabs(calculatedG - expectedS) > tol) {
                FAIL("Failed to reproduce g values "
...
TEST_CASE("NormalCLVModel_MonteCarloBSOptionPricing", "[NormalCLVModel]") {
    ...
    VanillaOption fdmOption(
            std::make_shared<CLVModelPayoff>(
                    payoff->optionType(), payoff->strike(), [t,g](Real r) { return g(t, r); }),
            exercise);
    ...
    if (std::fabs(calculated - expected) > tol) {
        FAIL("Failed to reproduce FDM vanilla option price "
                           << "\n    time:       " << maturity
                           << "\n    strike:     " << strike
                           << "\n    calculated: " << calculated
                           << "\n    expected:   " << expected);
...
TEST_CASE("NormalCLVModel_MoustacheGraph", "[NormalCLVModel]") {
    ...
    */
    ...
    while (maturities.back() < maturityDate)
        maturities.emplace_back(maturities.back() + Period(2, Weeks));
    ...
    for (Size u = 0; u < n; ++u) {
        ...
        if (std::fabs(calculated - expected[u]) > tol) {
            FAIL("Failed to reproduce Double no Touch prices"
                               << "\n    time:          " << maturityDate
