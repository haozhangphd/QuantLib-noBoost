        ...
        for (Date d=lim.first; d<=lim.second; d++) {
            if (d < inflationPeriod(todayMinusLag,iir->frequency()).first) {
                if (std::fabs(iir->fixing(d) - fixData[i]) > eps)
                    FAIL_CHECK("Fixings not constant within a period: "
                                << iir->fixing(d)
                                << ", should be " << fixData[i]);
            }
        }
...
TEST_CASE( "Inflation_ZeroTermStructure", "[Inflation]" ) {
    ...
    std::shared_ptr<ZeroInflationIndex> ii = std::dynamic_pointer_cast<ZeroInflationIndex>(iiUKRPI);
    ...
    for (Size i=0; i<LENGTH(zcData); i++) {
	    ...
	    if (std::fabs(helpers[i]->impliedQuote() - zcData[i].rate/100.0) >= eps)
		    FAIL("ZITS implied quote != instrument "
    ...
    for (Size i=0; i<testIndex.size();i++) {
        ...
        if (std::fabs(calc - ii->fixing(d,true))/10000.0 > eps)
            FAIL_CHECK("ZC index does not forecast correctly for date " << d
                        << " from base date " << bd
                        << " with fixing " << bf
                        << ", correct:  " << calc
    ...
    std::shared_ptr<Index> ind = std::dynamic_pointer_cast<Index>(ii);
    if (!ind)
	    FAIL("dynamic_pointer_cast to Index from InflationIndex failed");
    ...
    if (std::fabs(correctIndexed - calculatedIndexed) >= eps)
	    FAIL("IndexedCashFlow indexing wrong: " << calculatedIndexed << " vs correct = "
                  << correctIndexed);
    ...
    std::shared_ptr<ZeroInflationIndex> zii = std::dynamic_pointer_cast<ZeroInflationIndex>(ii);
    if (!zii)
	    FAIL("dynamic_pointer_cast to ZeroInflationIndex from UKRPI failed");
    ...
    if (fabs(nzcis.NPV())>=0.00001)
	    FAIL_CHECK("ZCIS does not reprice to zero "
                        << nzcis.NPV()
    ...
    for(int i=0;i<12;i++){
        if(std::fabs(fixing[i] - seasonalityFixing_1[i]) > eps) {
            FAIL_CHECK("Seasonality doesn't work correctly when seasonality factors are set = 1");
        }
    }
    ...
    for(int i=0;i<12;i++){
        if(std::fabs(expectedFixing[i] - seasonalityFixing_real[i]) > 0.01) {
            FAIL_CHECK("Seasonality doesn't work correctly when considering seasonality factors != 1 "
    ...
    for(int i=0;i<12;i++){
        if(std::fabs(seasonalityFixing_unset[i] - seasonalityFixing_1[i]) > eps) {
            FAIL_CHECK("UnsetSeasonality doesn't work correctly "
    ...
    std::shared_ptr<ZeroInflationIndex> iiyes
        = std::dynamic_pointer_cast<ZeroInflationIndex>(iiUKRPIyes);

    // now build the zero inflation curve
    // same data, bigger lag or it will be a self-contradiction
    Period observationLagyes = Period(3,Months);
    std::vector<std::shared_ptr<BootstrapHelper<ZeroInflationTermStructure> > > helpersyes =
    makeHelpers<ZeroInflationTermStructure,ZeroCouponInflationSwapHelper,
    ...
    for (Size i=0; i<LENGTH(zcData); i++) {
		...
		FAIL_CHECK("ZITS INTERPOLATED zeroRate != instrument "
                           ...
                           << " forceLinearInterpolation " << forceLinearInterpolation);
        if (std::fabs(helpersyes[i]->impliedQuote() - zcData[i].rate/100.0) >= eps)
		FAIL_CHECK("ZITS INTERPOLATED implied quote != instrument "
    ...
    for (Size i=0; i<testIndex.size();i++) {
        ...
        if (std::fabs(calc - iiyes->fixing(d)) > eps)
            FAIL_CHECK("ZC INTERPOLATED index does not forecast correctly for date " << d
                        << " from base date " << bd
                        << " with fixing " << bf
                        << ", correct:  " << calc
                        << ", fix: " << iiyes->fixing(d)
    ...
    std::shared_ptr<ZeroInflationIndex> ziiyes = std::dynamic_pointer_cast<ZeroInflationIndex>(iiyes);
    if (!ziiyes)
	    FAIL("dynamic_pointer_cast to ZeroInflationIndex from UKRPI-I failed");
    ...
    // ... and price it, should be zero
    if (fabs(nzcisyes.NPV())>=0.00001)
	    FAIL_CHECK("ZCIS-I does not reprice to zero "
                        << nzcisyes.NPV()
...
TEST_CASE( "Inflation_ZeroIndexFutureFixing", "[Inflation]" ) {
    ...
    if (std::fabs(fixing - sample_fixing) > 1e-12)
        FAIL_CHECK("Failed to retrieve correct fixing: "
                    << "\n    returned: " << fixing
    ...
    if (retrieved)
        FAIL_CHECK("Retrieved future fixing: "
                    << "\n    returned: " << fixing);
}
...
TEST_CASE( "Inflation_YYIndex", "[Inflation]" ) {
    ...
    if (yyeuhicp.name() != "EU YY_HICP"
        ...
        FAIL_CHECK("wrong year-on-year EU HICP data ("
                    << yyeuhicp.name() << ", "
                    << yyeuhicp.frequency() << ", "
                    << yyeuhicp.revised() << ", "
                    << yyeuhicp.interpolated() << ", "
    ...
    if (yyeuhicpr.name() != "EU YYR_HICP"
        ...
        FAIL_CHECK("wrong year-on-year EU HICPr data ("
                    << yyeuhicpr.name() << ", "
                    << yyeuhicpr.frequency() << ", "
                    << yyeuhicpr.revised() << ", "
                    << yyeuhicpr.interpolated() << ", "
    ...
    if (yyukrpi.name() != "UK YY_RPI"
        ...
        FAIL_CHECK("wrong year-on-year UK RPI data ("
                    << yyukrpi.name() << ", "
                    << yyukrpi.frequency() << ", "
                    << yyukrpi.revised() << ", "
                    << yyukrpi.interpolated() << ", "
    ...
    if (yyukrpir.name() != "UK YYR_RPI"
        ...
        FAIL_CHECK("wrong year-on-year UK RPIr data ("
                    << yyukrpir.name() << ", "
                    << yyukrpir.frequency() << ", "
                    << yyukrpir.revised() << ", "
                    << yyukrpir.interpolated() << ", "
    ...
    for (Size i=13; i<rpiSchedule.size();i++) {
        ...
        for (Date d=lim.first; d<=lim.second; d++) {
			...
			FAIL_CHECK("Non-interpolated fixings not constant within a period: "
                                   << calculated
                                   << ", should be "
                                   << expected);
                ...
                if (fabs(expectedYES-calculatedYES)>=eps)
			FAIL_CHECK("Error in interpolated fixings: expect "<<expectedYES
                                    <<" see " << calculatedYES
                                    <<" flat " << calculated
...
TEST_CASE( "Inflation_YYTermStructure", "[Inflation]" ) {
    ...
    for (Size j = 1; j < LENGTH(yyData); j++) {
        ...
        if (fabs(yyS2.NPV())>=eps)
		FAIL_CHECK("fresh yoy swap NPV!=0 from TS "
                           <<"swap quote for pt " << j
    ...
    for (Size k = 0; k < 14; k++) {
        ...
        if (fabs(yyS3.NPV())>= 20000.0)
		FAIL_CHECK("unexpected size of aged YoY swap, aged "
