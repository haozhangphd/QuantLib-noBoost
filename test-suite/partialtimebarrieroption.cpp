...
/*
 Copyright (C) 2014 Master IMAFA - Polytech'Nice Sophia - UniversitГ© de Nice Sophia Antipolis
...
TEST_CASE("PartialTimeBarrierOption_AnalyticEngine", "[PartialTimeBarrierOption]") {
    ...
    for (Size i=0; i<LENGTH(cases); ++i) {
        ...
        Real tolerance = 1e-4;
        if (error > tolerance)
            FAIL_CHECK("Failed to reproduce partial-time barrier option value"
                        << "\n    expected:   " << expected
