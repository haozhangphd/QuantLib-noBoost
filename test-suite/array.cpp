...
TEST_CASE( "Array_Construction", "[Array]" ) {
    ...
    Array a1;
    if (!a1.empty())
        FAIL_CHECK("default-initialized array is not empty "
                    "(size = " << a1.size() << ")");
    ...
    Array a2(size);
    if (a2.size() != size)
        FAIL_CHECK("array not of the required size"
                    << "\n    required:  " << size
                    << "\n    resulting: " << a2.size());
    ...
    for (i=0; i<size; ++i) {
        if (a3[i] != value)
            FAIL_CHECK(io::ordinal(i+1) << " element not with required value"
                        << "\n    required:  " << value
                        << "\n    resulting: " << a3[i]);
    }
    ...
    Array a5(a1);
    if (a5.size() != a1.size())
        FAIL_CHECK("copy not of the same size as original"
                    << "\n    original:  " << a1.size()
                    << "\n    copy:      " << a5.size());
    ...
    for (i=0; i<a3.size(); i++) {
        if (a6[i] != a3[i])
            FAIL_CHECK(io::ordinal(i+1) << " element of copy "
    ...
    for (i=0; i<size; i++) {
        if (temp2[i] != value)
            FAIL_CHECK(io::ordinal(i+1) << " element of disposable "
                        "not moved correctly"
                        << "\n    required:  " << value
                        << "\n    resulting: " << temp2[i]);
    }
    ...
    for (i=0; i<size; i++) {
        if (a7[i] != value)
            FAIL_CHECK(io::ordinal(i+1) << " element not moved correctly"
                        << "\n    required:  " << value
                        << "\n    resulting: " << a7[i]);
    }
    ...
    for (i=0; i < a10.size(); i++) {
        ...
        if (std::fabs(a10[i] -  calculated) >= 1e-5) {
            FAIL_CHECK("Array transform test failed " << a10[i] << " "
                        << calculated);
        }
    }
}
...
TEST_CASE( "Array_ArrayFunctions", "[Array]" ) {
    ...
    for (Size i=0; i < a.size(); ++i) {
        if (std::fabs(p[i]-std::pow(a[i], exponential)) > tol) {
            FAIL("Array function test Pow failed");
        }
        if (std::fabs(e[i]-std::exp(a[i])) > tol) {
            FAIL("Array function test Exp failed");
        }
        if (std::fabs(l[i]-std::log(a[i])) > tol) {
            FAIL("Array function test Log failed");
        }
        if (std::fabs(s[i]-std::sqrt(a[i])) > tol) {
            FAIL("Array function test Sqrt failed");
        }
    }
