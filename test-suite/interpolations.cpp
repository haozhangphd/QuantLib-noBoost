...
namespace {
    ...
    void checkValues(const char *type,
        ...
        while (xBegin != xEnd) {
            ...
            if (std::fabs(interpolated - *yBegin) > tolerance) {
                FAIL_CHECK(type << " interpolation failed at x = " << *xBegin
                                << QL_SCIENTIFIC
    ...
    void check1stDerivativeValue(const char *type,
        ...
        if (error > tolerance) {
            FAIL_CHECK(type << " interpolation first derivative failure\n"
                            << "at x = " << x
                            << "\n    interpolated value: " << interpolated
    ...
    void check2ndDerivativeValue(const char *type,
        ...
        Real error = std::fabs(interpolated - value);
        if (error > tolerance) {
            FAIL_CHECK(type << " interpolation second derivative failure\n"
                            << "at x = " << x
                            << "\n    interpolated value: " << interpolated
                            << "\n    expected value:     " << value
                            << QL_SCIENTIFIC
    ...
    void checkNotAKnotCondition(const char *type,
        ...
        if (std::fabs(c[0] - c[1]) > tolerance) {
            FAIL_CHECK(type << " interpolation failure"
    ...
    void checkSymmetry(const char *type,
        ...
        for (Real x = xMin; x < 0.0; x += 0.1) {
            ...
            if (std::fabs(y1 - y2) > tolerance) {
                FAIL_CHECK(type << " interpolation not symmetric"
                                << "\n    x = " << x
                                << "\n    g(x)  = " << y1
                                << "\n    g(-x) = " << y2
    ...
    auto errorFunction = [](auto f) { return ([f](Real x) { return std::pow(f(x) - std::exp(-x * x), 2); }); };

    template<class F>
    auto make_error_function(F f) { return errorFunction(f); }
...
TEST_CASE("Interpolation_SplineErrorOnGaussianValues", "[Interpolation]") {
    ...
    for (Size i = 0; i < LENGTH(points); i++) {
        ...
        if (std::fabs(result - tabulatedErrors[i]) > toleranceOnTabErr[i])
            FAIL_CHECK("Not-a-knot spline interpolation "
                               << "\n    sample points:      " << n
        ...
        if (std::fabs(result - tabulatedMCErrors[i]) > toleranceOnTabMCErr[i])
            FAIL_CHECK("MC Not-a-knot spline interpolation "
                               << "\n    sample points:      " << n
                               << "\n    norm of difference: " << result
                               << "\n    it should be:       "
                               << tabulatedMCErrors[i]);
...
TEST_CASE("Interpolation_SplineOnRPN15AValues", "[Interpolation]") {
    ...
    if (interpolated < 1.0) {
        FAIL_CHECK("Natural spline interpolation "
    ...
    interpolated = f(x_bad);
    if (interpolated < 1.0) {
        FAIL_CHECK("Clamped spline interpolation "
                           << "poor performance unverified\n"
                           << "at x = " << x_bad
                           << "\ninterpolated value: " << interpolated
                           << "\nexpected value > 1.0");
    ...
    interpolated = f(x_bad);
    if (interpolated > 1.0) {
        FAIL_CHECK("MC natural spline interpolation "
                           << "good performance unverified\n"
                           << "at x = " << x_bad
                           << "\ninterpolated value: " << interpolated
                           << "\nexpected value < 1.0");
    ...
    interpolated = f(x_bad);
    if (interpolated > 1.0) {
        FAIL_CHECK("MC clamped spline interpolation "
                           << "good performance unverified\n"
                           << "at x = " << x_bad
                           << "\ninterpolated value: " << interpolated
                           << "\nexpected value < 1.0");
...
TEST_CASE("Interpolation_SplineOnGenericValues", "[Interpolation]") {
    ...
    if (x35[0] > x35[1] || x35[1] > x35[2]) {
        FAIL_CHECK("Spline interpolation failure"
                           << "\nat x = " << 3.5
...
TEST_CASE("Interpolation_NonRestrictiveHymanFilter", "[Interpolation]") {
    ...
    if (std::fabs(interpolated - expected) > 1e-15) {
        FAIL_CHECK("MC not-a-knot spline"
    ...
    interpolated = f(zero);
    if (std::fabs(interpolated - expected) > 1e-15) {
        FAIL_CHECK("MC SecondDerivative spline"
                           << " interpolation failed at x = " << zero
                           << "\n    interpolated value: " << interpolated
                           << "\n    expected value:     " << expected
                           << "\n    error:              "
        ...
        if (std::fabs(volatilities[i] - calculatedVol) > tolerance)
            FAIL_CHECK(
                    "failed to calculate Sabr function at strike " << strikes[i]
                                                                   ...
                                                                   << "\n    error:      "
                                                                   << std::fabs(calculatedVol - volatilities[i]));
    }
...
TEST_CASE("Interpolation_KernelInterpolation", "[Interpolation]") {
        ...
        for (Size j = 0; j < yd.size(); ++j) {
            ...
            for (Size dIt = 0; dIt < deltaGrid.size(); ++dIt) {
                ...
                if (std::fabs(expectedVal - calcVal) > tolerance) {
                    ...
                    FAIL_CHECK("Kernel interpolation failed at x = "
                                       << deltaGrid[dIt]
                                       << QL_SCIENTIFIC
    ...
    ytd.emplace_back(ytd1);
    ytd.emplace_back(ytd2);
    ytd.emplace_back(ytd3);
...
TEST_CASE("Interpolation_KernelInterpolation2D", "[Interpolation]") {
    ...
    INFO("Testing kernel 2D interpolation...");
    ...
    for (Size i = 0; i < M.rows(); ++i) {
        for (Size j = 0; j < M.columns(); ++j) {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance) {
                ...
                FAIL_CHECK("2D Kernel interpolation failed at x = " << xVec[i]
                                                                    << ", y = " << yVec[j]
                                                                    << "\n    interpolated value: " << calcVal
                                                                    << "\n    expected value:     " << expectedVal
                                                                    << "\n    error:              "
    ...
    for (Size i = 0; i < M1.rows(); ++i) {
        for (Size j = 0; j < M1.columns(); ++j) {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance) {
                ...
                FAIL_CHECK("2D Epanechnkikov Kernel interpolation failed at x = " << xVec1[i]
                                                                                  << ", y = " << yVec1[j]
                                                                                  << "\n    interpolated value: "
                                                                                  << calcVal
                                                                                  << "\n    expected value:     "
    ...
    for (Size i = 0; i < M1.rows(); ++i) {
        for (Size j = 0; j < M1.columns(); ++j) {
            ...
            if (std::fabs(expectedVal - calcVal) > tolerance) {
                ...
                FAIL_CHECK("2D Epanechnkikov Kernel updated interpolation failed at x = " << xVec1[i]
                                                                                          << ", y = " << yVec1[j]
                                                                                          << "\n    interpolated value: "
                                                                                          << calcVal
                                                                                          << "\n    expected value:     "
    ...
    for (Size i = 5; i < 95; i += 10) {
        for (Size j = 5; j < 95; j += 10) {
            ...
            if (std::fabs(f_x - y[i] / 10 * std::cos(x[j])) > tol) {
                FAIL_CHECK("Failed to reproduce f_x");
            }
            if (std::fabs(f_xx + y[i] / 10 * std::sin(x[j])) > tol) {
                FAIL_CHECK("Failed to reproduce f_xx");
            }
            if (std::fabs(f_y - (std::sin(x[j]) / 10 - std::sin(y[i]))) > tol) {
                FAIL_CHECK("Failed to reproduce f_y");
            }
            if (std::fabs(f_yy + std::cos(y[i])) > tol) {
                FAIL_CHECK("Failed to reproduce f_yy");
            }
            if (std::fabs(f_xy - std::cos(x[j]) / 10) > tol) {
                FAIL_CHECK("Failed to reproduce f_xy");
            }
        }
    }
}
...
TEST_CASE("Interpolation_BicubicUpdate", "[Interpolation]") {
    ...
    if (std::fabs(old_result - new_result) < 0.5) {
        FAIL_CHECK("Failed to update bicubic spline");
    }
}
...
TEST_CASE("Interpolation_RichardsonExtrapolation", "[Interpolation]") {
    ...
    if (std::fabs(expected - calculated) > tol) {
        FAIL_CHECK("failed to reproduce Richardson extrapolation");
    }
...
TEST_CASE("Interpolation_NoArbSabrInterpolation", "[Interpolation]") {
    ...
    NoArbSabrSmileSection noarbSabr(expiry, forward,
                                    std::vector<Real>{initialAlpha, initialBeta, initialNu, initialRho});
    for (Size i = 0; i < strikes.size(); i++) {
    ...
    for (Size j = 1; j < methods_.size(); ++j) { // skip simplex (gets caught in some cases)
        for (Size i = 0; i < LENGTH(vegaWeighted); ++i) {
            for (Size k_a = 0; k_a < LENGTH(isAlphaFixed); ++k_a) {
                for (Size k_b = 0; k_b <
                                   1/*LENGTH(isBetaFixed)*/; ++k_b) { // keep beta fixed (all 4 params free is a problem for this kind of test)
                    for (Size k_n = 0; k_n < LENGTH(isNuFixed); ++k_n) {
                        for (Size k_r = 0; k_r < LENGTH(isRhoFixed); ++k_r) {
...
TEST_CASE("Interpolation_SabrSingleCases", "[Interpolation]") {
    ...
    if (s0.maxError() > 0.01 || s0.rmsError() > 0.01) {
        FAIL_CHECK("Sabr case #1 failed with max error ("
...
TEST_CASE("Interpolation_Transformations", "[Interpolation]") {
    ...
    for (Size i = 0; i < 1E6; ++i) {
        ...
        if (!close(z[0], y[0], N) || !close(z[1], y[1], N) || !close(z[2], y[2], N) ||
            !close(z[3], y[3], N))
            FAIL_CHECK("SabrInterpolation: direct(inverse("
                               << y[0] << "," << y[1] << "," << y[2] << "," << y[3]
                               << ")) = (" << z[0] << "," << z[1] << "," << z[2] << ","
        ...
        if (!close(z[0], y[0], N) || !close(z[1], y[1], N) || !close(z[2], y[2], N) ||
            !close(z[3], y[3], N))
            FAIL_CHECK("NoArbSabrInterpolation: direct(inverse("
                               << y[0] << "," << y[1] << "," << y[2] << "," << y[3]
                               << ")) = (" << z[0] << "," << z[1] << "," << z[2] << ","
                               << z[3] << "), difference is (" << z[0] - y[0] << ","
                               << z[1] - y[1] << "," << z[2] - y[2] << ","
...
TEST_CASE("Interpolation_LagrangeInterpolation", "[Interpolation]") {
    ...
    for (Size i = 0; i < 79; ++i) {
        ...
        if (std::isnan(calculated)
            ...
            FAIL("failed to reproduce the Lagrange interpolation"
                         << "\n    x         : " << xx
                         << "\n    calculated: " << calculated
    ...
    for (Size i = 1; i < n - 1; ++i) {
        for (Real z = x[i] - 100 * QL_EPSILON;
            ...
            if (std::isnan(calculated)
                ...
                FAIL("failed to reproduce the Lagrange interplation"
                             << "\n    x         : " << z
                             << "\n    calculated: " << calculated
                             << "\n    expected  : " << expected);
            }
...
TEST_CASE("Interpolation_BSplines", "[Interpolation]") {
    INFO("Testing B-Splines...");
    ...
    std::vector<std::tuple<Natural, Real, Real> > referenceValues{std::make_tuple(0, -0.95, 9.5238095238e-04),
    ...
    for (Size i = 0; i < referenceValues.size(); ++i) {
        //TODO-HAO: Remove this special case after VS 2017.3 is released
#ifndef _MSC_VER
        const auto [idx, x, expected ] = referenceValues[i];
#else
        Natural idx;
        Real x;
        Real expected;
        std::tie(idx, x, expected) = referenceValues[i];
#endif
        const Real calculated = bspline(idx, x);

        if (std::isnan(calculated)
            || std::fabs(calculated - expected) > tol) {
            FAIL("failed to reproduce the B-Spline value"
                         << "\n    i         : " << idx
                         << "\n    x         : " << x
                         << "\n    calculated: " << calculated
                         << "\n    expected  : " << expected
