...
namespace {
    ...
    struct CommonVars {
        ...
        Leg makeYoYCapFlooredLeg(Size which, const Date &startDate,
            ...
            switch (which) {
                case 0:
                    pricer = std::make_shared<BlackYoYInflationCouponPricer>(vol);
                    break;
                case 1:
                    pricer = std::make_shared<UnitDisplacedBlackYoYInflationCouponPricer>(vol);
                    break;
                case 2:
                    pricer = std::make_shared<BachelierYoYInflationCouponPricer>(vol);
                    break;
                default:
                    FAIL("unknown coupon pricer request: which = " << which
            ...
            for (Size i = 0; i < yoyLeg.size(); i++) {
                std::dynamic_pointer_cast<YoYInflationCoupon>(yoyLeg[i])->setPricer(pricer);
            }
...
TEST_CASE("InflationCapFlooredCoupon_Decomposition", "[InflationCapFlooredCoupon]") {
    ...
    std::shared_ptr < PricingEngine > engine =
            std::make_shared<DiscountingSwapEngine>(vars.nominalTS);
    ...
    if (error > tolerance) {
        FAIL_CHECK("YoY Floored Leg: gearing=1, spread=0%, strike=" << floorstrike * 100 <<
                                                                    "%\n" <<
                                                                    "  Floored Floating Leg NPV: " << npvFlooredLeg
                                                                    << "\n" <<
                                                                    "  Floating Leg NPV + Floor NPV: "
                                                                    << npvVanilla + npvFloor << "\n" <<
                                                                    "  Diff: " << error);
    }
