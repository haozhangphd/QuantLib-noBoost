...
TEST_CASE("CashFlows_DefaultSettlementDate", "[CashFlows]") {
    ...
    if (accruedPeriod == 0.0)
        FAIL_CHECK("null accrued period with default settlement date");

    Date::serial_type accruedDays = CashFlows::accruedDays(leg, false);
    if (accruedDays == 0)
        FAIL_CHECK("no accrued days with default settlement date");
...
TEST_CASE("CashFlows_IrregularFirstCouponReferenceDatesAtEndOfMonth", "[CashFlows]") {
    ...
    if (firstCoupon->referencePeriodStart() != Date(31, August, 2016))
        FAIL_CHECK("Expected reference start date at end of month, "
...
TEST_CASE("CashFlows_IrregularLastCouponReferenceDatesAtEndOfMonth", "[CashFlows]") {
    ...
    if (lastCoupon->referencePeriodEnd() != Date(31, August, 2018))
        FAIL_CHECK("Expected reference end date at end of month, "
