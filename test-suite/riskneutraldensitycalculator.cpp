...
TEST_CASE("RiskNeutralDensityCalculator_LocalVolatilityRND", "[RiskNeutralDensityCalculator]") {
    ...
    const std::shared_ptr<TimeGrid> timeGrid = std::make_shared<TimeGrid>(1.0, 101);
    ...
    const std::shared_ptr<DumasParametricVolSurface> dumasVolSurface =
        std::make_shared<DumasParametricVolSurface>(b1, b2, b3, b4, b5, spot, rTS, qTS);
    ...
    const std::shared_ptr<TimeGrid> dumasTimeGrid =
        std::make_shared<TimeGrid>(adaptiveGrid.begin(), adaptiveGrid.end());
    ...
    for (Size i=0; i < maturities.size(); ++i) {
        ...
        const std::shared_ptr<Exercise> exercise =
            std::make_shared<EuropeanExercise>(maturities[i]);
...
TEST_CASE("RiskNeutralDensityCalculator_BlackScholesWithSkew", "[RiskNeutralDensityCalculator]") {
    ...
    const std::shared_ptr<TimeGrid> timeGrid = std::make_shared<TimeGrid>(maturity, 51);
