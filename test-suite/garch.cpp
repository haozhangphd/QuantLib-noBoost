...
#define GARCH_CHECK(results, garch, member, tolerance) \
    if (std::fabs(results.member - garch.member()) > tolerance) { \
        FAIL_CHECK("Failed to reproduce expected " #member \
...
TEST_CASE("GARCH_Calibration", "[GARCH]") {
    ...
    GARCH_CHECK(calibrated, cgarch1, alpha, tolerance);
    GARCH_CHECK(calibrated, cgarch1, beta, tolerance);
    GARCH_CHECK(calibrated, cgarch1, omega, tolerance);
    GARCH_CHECK(calibrated, cgarch1, logLikelihood, tolerance);
    ...
    GARCH_CHECK(expected1, cgarch2, alpha, tolerance);
    GARCH_CHECK(expected1, cgarch2, beta, tolerance);
    GARCH_CHECK(expected1, cgarch2, omega, tolerance);
    GARCH_CHECK(expected1, cgarch2, logLikelihood, tolerance);

    // Optimization from this initial guess
    cgarch2.calibrate(ts);

    GARCH_CHECK(calibrated, cgarch2, alpha, tolerance);
    GARCH_CHECK(calibrated, cgarch2, beta, tolerance);
    GARCH_CHECK(calibrated, cgarch2, omega, tolerance);
    GARCH_CHECK(calibrated, cgarch2, logLikelihood, tolerance);
    ...
    GARCH_CHECK(expected2, cgarch3, alpha, tolerance);
    GARCH_CHECK(expected2, cgarch3, beta, tolerance);
    GARCH_CHECK(expected2, cgarch3, omega, tolerance);
    GARCH_CHECK(expected2, cgarch3, logLikelihood, tolerance);

    // Optimization from this initial guess
    cgarch3.calibrate(ts);

    GARCH_CHECK(calibrated, cgarch3, alpha, tolerance);
    GARCH_CHECK(calibrated, cgarch3, beta, tolerance);
    GARCH_CHECK(calibrated, cgarch3, omega, tolerance);
    GARCH_CHECK(calibrated, cgarch3, logLikelihood, tolerance);

    // Double optimization using type 1 and 2 initial guesses
    Garch11 cgarch4(ts,  Garch11::DoubleOptimization);
    cgarch4.calibrate(ts);

    GARCH_CHECK(calibrated, cgarch4, alpha, tolerance);
    GARCH_CHECK(calibrated, cgarch4, beta, tolerance);
    GARCH_CHECK(calibrated, cgarch4, omega, tolerance);
    GARCH_CHECK(calibrated, cgarch4, logLikelihood, tolerance);
    ...
    GARCH_CHECK(expected3, cgarch4, alpha, tolerance);
    GARCH_CHECK(expected3, cgarch4, beta, tolerance);
    GARCH_CHECK(expected3, cgarch4, omega, tolerance);
    GARCH_CHECK(expected3, cgarch4, logLikelihood, tolerance);
}
...
namespace {
    ...
    void check_ts(const std::pair<Date, Volatility> &x) {
        if (x.first.serialNumber() < 22834 || x.first.serialNumber() > 22843) {
            FAIL_CHECK("Failed to reproduce calculated GARCH time: "
                        << "\n    calculated: " << x.first.serialNumber()
                        << "\n    expected:   [22835, 22844]");
        }
        Real error =
            std::fabs(x.second - expected_calc[x.first.serialNumber()-22834]);
        if (error > tolerance) {
            FAIL_CHECK("Failed to reproduce calculated GARCH value at "
                        << x.first.serialNumber() << ": "
                        << "\n    calculated: " << x.second
                        << "\n    expected:   "
                        << expected_calc[x.first.serialNumber()-22834]);
        }
    }
