...
namespace {
    ...
    std::shared_ptr<MarketModelEvolver> makeMarketModelEvolver(
        ...
        switch (evolverType) {
            case Ipc:
                return std::make_shared<LogNormalFwdRateIpc>(marketModel, generatorFactory,
                                                             numeraires, initialStep);
            case Balland:
                return std::make_shared<LogNormalFwdRateBalland>(marketModel, generatorFactory,
                                                                 numeraires, initialStep);
            case Pc:
                return std::make_shared<LogNormalFwdRatePc>(marketModel, generatorFactory,
                                                            numeraires, initialStep);
            case NormalPc:
                return std::make_shared<NormalFwdRatePc>(marketModel, generatorFactory,
                                                         numeraires, initialStep);
            default:
                QL_FAIL("unknown MarketModelEvolver type");
        }
    ...
    void checkMultiProductCompositeResults(const SequenceStatisticsInc &stats,
        ...
        if (nbOfResults != results.size())
            FAIL_CHECK("mismatch between the size of the result and the \
                            number of results");
    ...
    void checkCallableSwap(const SequenceStatisticsInc &stats,
        ...
        if (swapError > tolerance || bermudanNPV < 0.0 ||
            ...
            INFO(config);  // detailed error info below
        if (swapError > tolerance)
            FAIL_CHECK("agreement between payer and receiver swap failed:"
                               "\n    payer swap:    " << payerNPV <<
                                                       "\n    receiver swap: " << receiverNPV <<
                                                       "\n    error:         " << swapError <<
                                                       "\n    tolerance:     " << tolerance);
        if (bermudanNPV < 0.0)
            FAIL_CHECK("negative bermudan option value:"
                               "\n    bermudan:          " << bermudanNPV);
        if (callableNPV < receiverNPV)
            FAIL_CHECK("callable receiver less valuable than plain receiver:"
                               "\n    receiver swap:     " << receiverNPV <<
                                                           "\n    callable:          " << callableNPV);
        if (callableError > tolerance)
            FAIL_CHECK("agreement between receiver+bermudan and callable failed:"
                               "\n    receiver swap:     " << receiverNPV <<
                                                           "\n    bermudan:          " << bermudanNPV <<
        ...
        if (printReport_) {
            INFO(std::setprecision(2) <<
                                      ...
                                      "\n    bermudan:          " << io::rate(bermudanNPV) << " +/- "
                                      << io::rate(stats.errorEstimate()[2]) <<
...
TEST_CASE("MarketModel_OneStepForwardsAndOptionlets", "[MarketModel]") {
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        ...
        optionletPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, todaysForwards[i]);
...
TEST_CASE("MarketModel_OneStepNormalForwardsAndOptionlets", "[MarketModel]") {
...
TEST_CASE("MarketModel_InverseFloater", "[MarketModel]") {
            ...
            for (Size k = 0; k < LENGTH(measures); k++) {
                ...
                for (Size i = 0; i < LENGTH(evolvers); i++) {
                    ...
                    if (fabs(errorInSds) > 4.0)
                        FAIL("Inverse floater product has price error equal to " << errorInSds << " sds . Price "
                                                                                 << truePrice << " MC price "
                                                                                 << stats->mean()[0] << " \n");
                    ...
                    if (fabs(stats->mean()[0] - stats->mean()[1]) > numericalTolerance)
                        FAIL("Inverse floater and wrapper pathwise inverse floater do not agree:" << stats->mean()[0]
                                                                                                  << "  "
                                                                                                  << stats->mean()[1]);
...
void addForwards(MultiProductComposite &product,
    ...
    subProductExpectedValues.emplace_back(SubProductExpectedValues("Forward"));
    subProductExpectedValues.back().errorThreshold = 2.50;
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        subProductExpectedValues.back().values.emplace_back(
...
void addOptionLets(MultiProductComposite &product,
    ...
    // computing and storing expected values
    subProductExpectedValues.emplace_back(SubProductExpectedValues("Caplet"));
    subProductExpectedValues.back().errorThreshold = 2.50;
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        subProductExpectedValues.back().values.emplace_back(
                BlackCalculator(displacedPayoffs[i],
...
void addCoinitialSwaps(MultiProductComposite &product,
    ...
    // computing and storing expected values
    subProductExpectedValues.emplace_back(SubProductExpectedValues("coinitial swap"));
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        coinitialSwapValue += (todaysForwards[i] - fixedRate)
                              * accruals[i] * todaysDiscounts[i + 1];
        subProductExpectedValues.back().values.emplace_back(coinitialSwapValue);
    }
}
...
void addCoterminalSwapsAndSwaptions(MultiProductComposite &product,
    ...
    for (Size i = 0; i < payoffs.size(); ++i)
        payoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, todaysForwards[i]);
    ...
    subProductExpectedValues.emplace_back(SubProductExpectedValues("coterminal swap"));
    subProductExpectedValues.back().testBias = false;
    subProductExpectedValues.back().errorThreshold = 2.32;
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        ...
        subProductExpectedValues.back().values.emplace_back(expectedNPV);
    }
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        ...
        subProductExpectedValues.back().values.emplace_back(expectedSwaption);
    }
}


TEST_CASE("MarketModel_AllMultiStepProducts", "[MarketModel]") {
...
TEST_CASE("MarketModel_PeriodAdapter", "[MarketModel]") {

    INFO("Testing period-adaptation routines in LIBOR market model...");
    ...
    for (Size i = 0; i < numberBigRates; ++i) {
        optionletPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, bigRateCS.forwardRate(i));
        swaptionPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, bigRateCS.coterminalSwapRate(i));
        displacedOptionletPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, bigRateCS.forwardRate(i) + displacement);
        displacedSwaptionPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, bigRateCS.coterminalSwapRate(i) + displacement);
    ...
    std::shared_ptr < MarketModel > adaptedforwardModel(std::make_shared<FwdPeriodAdapter>(originalModel,
                                                                                           period,
                                                                                           offset,
                                                                                           newDisplacements));

    std::shared_ptr < MarketModel > adaptedSwapModel = std::make_shared<FwdToCotSwapAdapter>(adaptedforwardModel);
    ...
    for (Size i = 0; i < numberBigRates; ++i) {
        if (fabs(capletErrorsInSds[i]) > capletTolerance) {
            FAIL(io::ordinal(i + 1) << "caplet , approx price " <<
    ...
    for (Size i = 0; i < numberBigRates; ++i) {
        if (fabs(swaptionErrorsInSds[i]) > swaptionTolerance) {
            FAIL(io::ordinal(i + 1) << "swaption, approx price " <<
...
TEST_CASE("MarketModel_CallableSwapNaif", "[MarketModel]") {

    INFO("Pricing callable swap with naif exercise strategy in a LIBOR market model...");
    ...
    //    exerciseTimes.emplace_back(rateTimes[i]);
            ...
            for (Size k = 0; k < LENGTH(measures); k++) {
                ...
                for (Size i = 0; i < LENGTH(evolvers) - stop; i++) {
                    ...
                    for (Size n = 0; n < 1; n++) {
                        ...
                        if (printReport_)
                            INFO("    upper bound delta: " << io::rate(delta) << " +- " << io::rate(deltaError));
...
TEST_CASE("MarketModel_CallableSwapLS", "[MarketModel]") {

    INFO("Pricing callable swap with Longstaff-Schwartz exercise strategy in a LIBOR market model...");
...
void CallableSwapAnderson(
        MarketModelType marketModelType, unsigned testedFactor) {

    INFO("Pricing callable swap with Anderson exercise "
    ...
    for (Size k = 0; k < LENGTH(measures); k++) {
        ...
        for (Size i = 0; i < LENGTH(evolvers) - stop; i++) {
            for (Size n = 0; n < 1; n++) {
                ...
                config <<
                       marketModelTypeToString(marketModelType) << ", " <<
                       factors << (factors > 1 ? (factors == todaysForwards.size() ? " (full) factors, " : " factors, ")
                                               : " factor,") <<
                       measureTypeToString(measures[k]) << ", " <<
                       evolverTypeToString(evolvers[i]) << ", " <<
                       "MT BGF";
                ...
                Real firstPassValue = genericEarlyExerciseOptimization(
                        collectedData, parametricForm, parameters, ec, om) *
                                      initialNumeraireValue;
                if (printReport_)
                    INFO("    initial estimate:  " << io::rate(firstPassValue));
...
TEST_CASE("MarketModel_CallableSwapAnderson", "[MarketModel]") {
    MarketModelType marketModels[] = {
            ExponentialCorrelationFlatVolatility,
            ExponentialCorrelationAbcdVolatility};
    ...
    for (Size j = 0; j < LENGTH(marketModels); j++) {
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m)
            CallableSwapAnderson(marketModels[j], testedFactors[m]);
    }
}

TEST_CASE("MarketModel_Greeks", "[MarketModel]") {

    INFO("Testing caplet greeks in a lognormal forward rate market model using partial proxy simulation...");
    ...
    std::vector<std::shared_ptr<StrikedTypePayoff> >
            displacedPayoffs(todaysForwards.size());
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        payoffs[i] = std::make_shared<CashOrNothingPayoff>(Option::Call, todaysForwards[i], 0.01);
        displacedPayoffs[i] = std::make_shared<CashOrNothingPayoff>(Option::Call, todaysForwards[i] + displacement, 0.01);
    }
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            for (Size k = 0; k < LENGTH(measures); k++) {
                ...
                for (Size n = 0; n < 1; n++) {
                    ...
                    std::shared_ptr < MarketModelEvolver > evolver = std::make_shared<LogNormalFwdRateEuler>(marketModel, generatorFactory, numeraires);
                    ...
                    for (Size i = 0; i < evolution.evolutionTimes().size(); ++i) {
                        startIndexOfConstraint.emplace_back(i);
                        endIndexOfConstraint.emplace_back(i + 1);
                    }


                    std::vector<
                            std::vector<std::shared_ptr<ConstrainedEvolver> > >
                            constrainedEvolvers;
                    ...
                    deltaGammaEvolvers.emplace_back(
                            std::make_shared<LogNormalFwdRateEulerConstrained>(marketModel,
                                                                     generatorFactory,
                                                                     numeraires));
                    ...
                    constrainedEvolvers.emplace_back(deltaGammaEvolvers);
                    diffWeights.emplace_back(deltaGammaWeights);
                    greekStats.emplace_back(deltaGammaStats);

                    constrainedEvolvers.emplace_back(vegaEvolvers);
                    diffWeights.emplace_back(vegaWeights);
                    greekStats.emplace_back(vegaStats);
...
TEST_CASE("MarketModel_PathwiseGreeks", "[MarketModel]") {

    INFO("Testing caplet deltas in a lognormal forward rate market model using pathwise method...");
                ...
                for (Size k = 0; k < LENGTH(measures); k++) {
                    ...
                    for (Size n = 0; n < 1; n++) {
...

                        {
                            ...
                            PathwiseAccountingEngine accountingengine(std::shared_ptr < LogNormalFwdRateEuler >
                        ...
                        for (Size i = 0; i < product->numberOfProducts(); ++i) {
                            ...
                            if (fabs(priceErrorInSds) > errorTheshold) {
                                INFO("Caplet " << i << " price " << prices[i] << " model price " << modelPrices[i]
                                               << "   Standard error: " << priceErrors[i] << " errors in sds: "
                                               << priceErrorInSds);
                        ...
                        if (numberErrors > 0)
                            FAIL("Pathwise greeks test has " << numberErrors << "\n");
                    }
                }
            }
        }
...
TEST_CASE("MarketModel_PathwiseVegas", "[MarketModel]") {
    ...
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        ...
        //CashOrNothingPayoff(Option::Call, todaysForwards[i], 0.01));
        displacedPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, todaysForwards[i] + displacement);
        //CashOrNothingPayoff(Option::Call, todaysForwards[i]+displacement, 1.01));
    }
    ...
    for (Size j = 0; j < LENGTH(marketModels); j++) {
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            for (Size k = 0; k < marketModel->numberOfSteps(); ++k)
                pseudoRoots.emplace_back(marketModel->pseudoRoot(k));
            ...
            if (numberSwaptionPseudoFailures > 0)
                FAIL_CHECK("swaption pseudo test failed " << numberSwaptionPseudoFailures << " times");
        }
    }
    ...
    for (Size j = 0; j < LENGTH(marketModels); j++) {
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            if (numberCapPseudoFailures > 0)
                FAIL_CHECK("cap pseudo test failed for prices " << numberCapPseudoFailures << " times");

            if (numberCapImpVolFailures > 0)
                FAIL_CHECK("cap pseudo test failed for implied vols " << numberCapImpVolFailures << " times");
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
                        ...
                        for (Size l = 0; l < evolution.numberOfRates(); ++l)
                            for (Size f = 0; f < factors; ++f) {
                                ...
                                CapPseudoDerivative upDerivative(
                                        std::shared_ptr < MarketModel > (std::make_shared<PseudoRootFacade>(bumpedUp)),
                                        capStrike,
                                        startIndex,
                                        endIndex, initialNumeraireValue);
                                ...
                                CapPseudoDerivative downDerivative(
                                        ...
                                        (std::make_shared<PseudoRootFacade>(bumpedDown)),
                                        capStrike,
                                        startIndex,
                                        endIndex, initialNumeraireValue);
            ...
            if (numberCapVolPseudoFailures > 0)
                FAIL_CHECK("cap pseudo test failed for implied vols " << numberCapVolPseudoFailures << " times");
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            for (Size k = 0; k < evolution.numberOfRates(); ++k) {
                for (Size f = 0; f < factors; ++f) {
                    ...
                    pseudoBumps.emplace_back(modelBump);
                    modelBump[k][f] = -bumpSizeNumericalDifferentiation;
                    pseudoBumpsDown.emplace_back(modelBump);
                }
            }
            ...
            for (Size l = 0; l < evolution.numberOfSteps(); ++l) {
                vegaBumps.emplace_back(std::vector<Matrix>());
            ...
            for (Size k = 0; k < LENGTH(measures); k++) {
                ...
                for (Size l = 0; l < evolution.numberOfSteps(); ++l) {
                    ...
                    testees.emplace_back(RatePseudoRootJacobian(pseudoRoot,
                    ...
                    testees2.emplace_back(RatePseudoRootJacobianAllElements(pseudoRoot,
                                                                            evolution.firstAliveRate()[l],
                                                                            numeraires[l],
                                                                            evolution.rateTaus(),
                                                                            marketModel->displacements()
                    ...
                    testers.emplace_back(RatePseudoRootJacobianNumerical(pseudoRoot,
                                                                         evolution.firstAliveRate()[l],
                                                                         numeraires[l],
                                                                         evolution.rateTaus(),
                                                                         pseudoBumps,
                    ...
                    testersDown.emplace_back(RatePseudoRootJacobianNumerical(pseudoRoot,
                                                                             evolution.firstAliveRate()[l],
                                                                             numeraires[l],
                                                                             evolution.rateTaus(),
                                                                             pseudoBumpsDown,
            ...
            for (Size deflate = 0; deflate < 2; ++deflate) {
                ...
                for (Size k = 0; k < LENGTH(measures); k++) {
...

                    {
                        ...
                        PathwiseVegasAccountingEngine accountingengine(std::shared_ptr < LogNormalFwdRateEuler >
                                                                       (std::make_shared<LogNormalFwdRateEuler>(
                                                                               evolver)), // method relies heavily on LMM Euler
                    ...
                    for (Size r = 0; r < truePrices.size(); ++r) {
                        truePrices[r] = BlackCalculator(displacedPayoffs[r], todaysForwards[r],
                                                        sqrt(totalCovariance[r][r]),
                                                        todaysDiscounts[r + 1] *
                                                        (rateTimes[r + 1] - rateTimes[r])).value();
                    }
                    ...
                    for (Size bb = 0; bb < vegaBumps[0].size(); ++bb) {
                        ...
                        for (Size r = 0; r < truePrices.size(); ++r) {
                            ...
                            bumpedPrices[r] = BlackCalculator(displacedPayoffs[r], todaysForwards[r],
                                                              sqrt(variances[r]),
                                                              todaysDiscounts[r + 1] *
                                                              (rateTimes[r + 1] - rateTimes[r])).value();
                    ...
                    for (Size i = 0; i < productToUse2->numberOfProducts(); ++i) {
                        ...
                        if (fabs(priceDiff) > 5 * priceErrors2[i])  // two sets of standard error
                            FAIL("pathwise accounting engine and pathwise vegas accounting engine not in perfect agreement for price.\n product "
            ...
            if (numberDeflatedErrors + numberUndeflatedErrors > 0)
                FAIL("Model pathwise vega test for caplets fails : " << numberDeflatedErrors << " deflated errors and "
                                                                     << numberUndeflatedErrors
                                                                     << " undeflated errors , biggest error in SEs is "
                                                                     << biggestError << "\n");
...

            {
                ...
                for (Size k = 0; k < LENGTH(measures); k++) {
...

                    {
                        ...
                        PathwiseVegasOuterAccountingEngine accountingengine(std::shared_ptr < LogNormalFwdRateEuler >
                                                                            (std::make_shared<LogNormalFwdRateEuler>(
                                                                                    evolver2)), // method relies heavily on LMM Euler
                                                                            capsDeflated,
                                                                            marketModel, // we need pseudo-roots and displacements
                                                                            vegaBumps,
...

                    {
                        ...
                        if (numberMeanFailures > 0)
                            FAIL("Comparison of Pathwise vegas accounting engine and PathwiseVegasOuterAccountingEngine yields discrepancies:"
                                         << numberMeanFailures
                                         << "  out of "
                                         << values.size());
...
TEST_CASE("MarketModel_PathwiseMarketVegas", "[MarketModel]") {

    INFO("Testing pathwise market vegas in a lognormal forward rate market model...");
    ...
    std::vector<std::shared_ptr<StrikedTypePayoff> >
            displacedPayoffs(todaysForwards.size());
    for (Size i = 0; i < todaysForwards.size(); ++i) {
        payoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, cs.coterminalSwapRate(i));

        displacedPayoffs[i] = std::make_shared<PlainVanillaPayoff>(Option::Call, cs.coterminalSwapRate(i) + displacement);
    ...
    for (Size j = 0; j < LENGTH(marketModels); j++) {
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            for (Size i = 0; i < caps.size(); ++i) {
                ...
                if (fabs(thisErrorInSds) > 4) {
                    INFO(" MC cap vega: " << vegasMatrix[i][i + swaptions.size()] << " Analytic cap vega:"
                                          << analyticVegas[i] << " Error in sds:" << thisErrorInSds << "\n");
                    ++numberDiagonalFailures;
                }
            ...
            if (offDiagonalFailures + numberDiagonalFailures > 0)
                FAIL("Pathwise market vega test fails for caps: " << offDiagonalFailures << " off diagonal failures \n "
                                                                  << " and " << numberDiagonalFailures
                                                                  << " on the diagonal.");
...
TEST_CASE("MarketModel_AbcdVolatilityIntegration", "[MarketModel]") {

    INFO("Testing Abcd-volatility integration...");
...
TEST_CASE("MarketModel_AbcdVolatilityCompare", "[MarketModel]") {

    INFO("Testing different implementations of Abcd-volatility...");
...
TEST_CASE("MarketModel_AbcdVolatilityFit", "[MarketModel]") {

    INFO("Testing Abcd-volatility fit...");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
TEST_CASE("MarketModel_StochVolForwardsAndOptionlets", "[MarketModel]") {
    ...
    std::shared_ptr < MarketModelVolProcess > volProcess = std::make_shared<SquareRootAndersen>(meanLevel, reversionSpeed, volVar, v0, evolution.evolutionTimes(), numberSubSteps, w1, w2, cutPoint);
        ...
        for (Size m = 0; m < LENGTH(testedFactors); ++m) {
            ...
            for (Size k = 0; k < LENGTH(measures); k++) {
                ...
                for (Size n = 0; n < 1; n++) {
                    ...
                    std::shared_ptr < MarketModelEvolver > evolver(std::make_shared<SVDDFwdRatePc>(marketModel,
                                                                                                   generatorFactory,
                                                                                                   volProcess,
                    ...
                    for (Size i = 0; i < accruals.size(); ++i) {
                        ...
                        if (fabs(errorSds) > 3.5)
                            FAIL("error in sds: " << errorSds << " for forward " << i << " in SV LMM test. True value:"
                                                  << trueValue << ", actual value: " << results[i]
...
//                                  sqrt(marketModel->totalCovariance(i)[i][i]/evolution.evolutionTimes()[i]);
                        ...
                        std::shared_ptr < StrikedTypePayoff > payoff =
                                std::make_shared<PlainVanillaPayoff>(Option::Call, forwardStrikes[i]);
...
//                                             AnalyticHestonEngine::Integration::gaussLobatto(1e-8, 1e-8),
                        ...
                        if (fabs(errorSds) > 4)
                            FAIL("error in sds: " << errorSds << " for caplet " << i << " in SV LMM test. True value:"
                                                  << trueValue << ", actual value: " << results[i + accruals.size()]
                                                  << " , standard error " << errors[i]);
...
TEST_CASE("MarketModel_DriftCalculator", "[MarketModel]") {

    // Test full factor drift equivalence between compute() and
    // computeReduced()

    INFO("Testing drift calculation...");
    ...
    for (Size k = 0; k < LENGTH(marketModels); ++k) {   // loop over market models
        ...
        for (Size j = 0; j < numberOfSteps; ++j) {     // loop over steps
            ...
            //INFO(io::ordinal(j+1) << " pseudoroot:\n" << A);
...
TEST_CASE("MarketModel_IsInSubset", "[MarketModel]") {

    // Performance test for isInSubset function (temporary)

    INFO("Testing isInSubset function...");

    setup();

    Size dim = 100;
    std::vector<Time> set, subset;
    for (Size i = 0; i < dim; i++) set.emplace_back(i * 1.0);
    for (Size i = 0; i < dim; i++) subset.emplace_back(dim + i * 1.0);
...
TEST_CASE("MarketModel_AbcdDegenerateCases", "[MarketModel]") {
    INFO("Testing abcd degenerate cases...");
...
TEST_CASE("MarketModel_Covariance", "[MarketModel]") {
    INFO("Testing market models covariance...");
    ...
    evolTimes4.emplace_back(0.3);
    evolTimes4.emplace_back(1.3);
    evolTimes4.emplace_back(2.0);
    evolTimes4.emplace_back(4.5);
    evolTimes4.emplace_back(8.2);

    evolTimes.emplace_back(evolTimes1);
    evolTimes.emplace_back(evolTimes2);
    evolTimes.emplace_back(evolTimes3);
    evolTimes.emplace_back(evolTimes4);
    ...
    for (Size k = 0; k < modelNames.size(); k++) {
        for (Size l = 0; l < evolNames.size(); l++) {
            ...
            switch (k) {
                case 0:
                    model = std::make_shared<FlatVol>(vols, corr, evolution, n - 1, rates, displ);
                    break;
                case 1:
                    model = std::make_shared<AbcdVol>(1.0, 0.0, 1.0E-50, 0.0, ks,
                                                      corr, evolution, n - 1, rates, displ);
                    break;
                default:
                    FAIL("Unknown model " << modelNames[k]);
            }
            if (model) {
