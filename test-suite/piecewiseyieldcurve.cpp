...
namespace {
    ...
    void testCurveConsistency(CommonVars& vars,
        ...
        for (Size i=0; i<vars.bonds; i++) {
            ...
            if (error > tolerance) {
                FAIL_CHECK(io::ordinal(i+1) << " bond failure:" <<
                            std::setprecision(8) <<
        ...
        for (Size i=0; i<vars.fras; i++) {
            ...
            if (std::fabs(expectedRate-estimatedRate) > tolerance) {
                FAIL_CHECK(io::ordinal(i+1) << " FRA failure:" <<
                            std::setprecision(8) <<
                            "\n  estimated rate: " << io::rate(estimatedRate) <<
                            "\n  expected rate:  " << io::rate(expectedRate));
            }
        ...
        for (Size i = 0; i<vars.immFuts; i++) {
            ...
            if (std::fabs(expectedRate - estimatedRate) > tolerance) {
                FAIL_CHECK(io::ordinal(i + 1) << " IMM futures failure:" <<
                    std::setprecision(8) <<
                    "\n  estimated rate: " << io::rate(estimatedRate) <<
                    "\n  expected rate:  " << io::rate(expectedRate));
            }
        ...
        for (Size i = 0; i<vars.asxFuts; i++) {
            ...
            if (std::fabs(expectedRate - estimatedRate) > tolerance) {
                FAIL_CHECK(io::ordinal(i + 1) << " ASX futures failure:" <<
                    std::setprecision(8) <<
                    "\n  estimated rate: " << io::rate(estimatedRate) <<
                    "\n  expected rate:  " << io::rate(expectedRate));
            }
    ...
    void testBMACurveConsistency(CommonVars& vars,
        ...
        for (Size i=0; i<vars.bmas; i++) {
            ...
            if (error > tolerance) {
                FAIL_CHECK(bmaData[i].n << " year(s) BMA swap:\n"
                            << std::setprecision(8)
...
TEST_CASE( "PiecewiseYieldCurve_Observability", "[PiecewiseYieldCurve]" ) {
    ...
    for (Size i=0; i<vars.deposits+vars.swaps; i++) {
        ...
        if (!f.isUp())
            FAIL("Observer was not notified of underlying rate change");
        if (vars.termStructure->discount(testTime,true) == discount)
            FAIL("rate change did not trigger recalculation");
        vars.rates[i]->setValue(vars.rates[i]->value()/1.01);
    }
    ...
    Settings::instance().evaluationDate() = vars.today;
    if (f.isUp())
        FAIL("Observer was notified of date change"
...
TEST_CASE( "PiecewiseYieldCurve_LiborFixing", "[PiecewiseYieldCurve]" ) {
    ...
    for (Size i=0; i<vars.swaps; i++) {
        ...
        Real tolerance = 1.0e-9;
        if (std::fabs(expectedRate-estimatedRate) > tolerance) {
            FAIL_CHECK("before LIBOR fixing:\n"
                        << swapData[i].n << " year(s) swap:\n"
                        << std::setprecision(8)
                        << "    estimated rate: "
    ...
    if (!f.isUp())
        FAIL_CHECK("Observer was not notified of rate fixing");
...
TEST_CASE( "PiecewiseYieldCurve_JpyLibor", "[PiecewiseYieldCurve]" ) {
    ...
    for (Size i=0; i<vars.swaps; i++) {
        ...
        if (error > tolerance) {
            FAIL_CHECK(swapData[i].n << " year(s) swap:\n"
                        << std::setprecision(8)
                        << "\n estimated rate: " << io::rate(estimatedRate)
                        << "\n expected rate:  " << io::rate(expectedRate)
                        << "\n error:          " << io::rate(error)
...
namespace {
    ...
    void testCurveCopy(CommonVars& vars,
        ...
        if (!close(r1, r2)) {
            FAIL_CHECK("failed to link original and copied curve");
        }
        ...
        if (close(r1, r3)) {
            FAIL_CHECK("failed to modify original curve");
        }
        if (!close(r2,r4)) {
            FAIL_CHECK(
...
TEST_CASE( "PiecewiseYieldCurve_SwapRateHelperLastRelevantDate", "[PiecewiseYieldCurve]" ) {
    ...
    PiecewiseYieldCurve<Discount, LogLinear> curve(today, std::vector<std::shared_ptr<RateHelper> >(1, helper),
                                                   Actual365Fixed());
    CHECK_NOTHROW(curve.discount(1.0));
}
