...
TEST_CASE("NoArbSabr_ConsistencyWithHagan", "[NoArbSabr]") {
    ...
    SabrSmileSection sabr(tau,f,std::vector<Real>{alpha, beta, nu, rho});
    NoArbSabrSmileSection noarbsabr(tau,f,std::vector<Real>{alpha, beta, nu, rho});

    Real absProb=noarbsabr.model()->absorptionProbability();
    if( absProb > 1E-10 || absProb < 0.0 )
        FAIL_CHECK("absorption probability should be close to zero, but is " << absProb);
    ...
    while (strike < 0.15) {
        ...
        if (std::fabs(sabrPrice - noarbsabrPrice) > 1e-5)
            FAIL_CHECK("incosistent Hagan price ("
        ...
        if (std::fabs(sabrDigital - noarbsabrDigital) > 1e-3)
            FAIL_CHECK("incosistent Hagan digital ("
        ...
        if (std::fabs(sabrDensity - noarbsabrDensity) > 1e-0)
            FAIL_CHECK("incosistent Hagan density ("
