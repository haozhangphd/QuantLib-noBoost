...
public:
    ...
    TestFunction(Real(*f)(const Array&), Array(*fs)(const Array&) = NULL) : f_(f), fs_(fs) {}
...
void testGaussianSA(Size dimension, Size maxSteps, Size staticSteps, Real initialTemp,
                    ...
                    std::shared_ptr<OptimizationMethod> localOptimizer = std::make_shared<LevenbergMarquardt>()){
...
int main(int, char* []) {
	...
	std::chrono::time_point<std::chrono::steady_clock> startT =  std::chrono::steady_clock::now();
        ...
        elapsed = static_cast<double>((endT - startT).count()) / 1.0e9;
