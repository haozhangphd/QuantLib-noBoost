...
void printBasket(
        const std::vector<std::shared_ptr<CalibrationHelper> > &basket) {
...
public:
    void start() { timer_ = std::chrono::steady_clock::now(); }

    void stop() {
        std::chrono::time_point<std::chrono::steady_clock> endT = std::chrono::steady_clock::now();
        elapsed_ = static_cast<double>((endT - timer_).count()) / 1.0e9;
    }
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        timer.start();
        std::vector<std::shared_ptr<CalibrationHelper> > basket =
