...
namespace QuantLib {
    ...
    class BasketGeneratingEngine {
      ...
      private:
        ...
        const std::shared_ptr<Gaussian1dModel> onefactormodel_;
        ...
        class MatchHelper : public CostFunction {
          public:
            MatchHelper(const VanillaSwap::Type type, const Real npv,
                        ...
                        const std::shared_ptr<SwapIndex> &indexBase,
            ...
            Real NPV(std::shared_ptr<VanillaSwap> swap, Real fixedRate,
                ...
                for (Size i = 0; i < swap->floatingLeg().size(); i++) {
                    std::shared_ptr<IborCoupon> c =
                        std::dynamic_pointer_cast<IborCoupon>(
                            swap->floatingLeg()[i]);
                    npv +=
            ...
            Array values(const Array &v) const {
                // transformations
