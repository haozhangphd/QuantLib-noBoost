    ...
    */
    ...
    inline void ReplicatingVarianceSwapEngine::computeOptionWeights(
        ...
        switch (type) {
          case Option::Call:
            ...
            strikes.emplace_back(strikes.back() + dk_);
            break;
          case Option::Put:
            std::sort(strikes.begin(), strikes.end(), std::greater<Real>());
            strikes.emplace_back(std::max(strikes.back() - dk_, 0.0));
            break;
          default:
        ...
        for (std::vector<Real>::const_iterator k=strikes.begin();
            ...
            std::shared_ptr<StrikedTypePayoff> payoff =
                                            std::make_shared<PlainVanillaPayoff>(type, *k);
            if ( k == strikes.begin() )
                optionWeights.emplace_back(std::make_pair(payoff,slope));
            else
                optionWeights.emplace_back(
    ...
    Real ReplicatingVarianceSwapEngine::computeReplicatingPortfolio(
        ...
        std::shared_ptr<Exercise> exercise =
                               std::make_shared<EuropeanExercise>(arguments_.maturityDate);
