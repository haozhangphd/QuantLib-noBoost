...
namespace QuantLib {
    ...
    namespace {
        ...
        public:
            integrand1(Real c_inf, std::function<Real(Real)> f)
                    : c_inf_(c_inf), f_(f) {}
        ...
        private:
            ...
            const std::function<Real(Real)> f_;
        public:
            integrand2(Real c_inf, std::function<Real(Real)> f)
                    : c_inf_(c_inf), f_(f) {}
        ...
        public:
            integrand3(Real c_inf, std::function<Real(Real)> f)
                    : int_(c_inf, f) {}
    ...
    AnalyticHestonEngine::Integration::Integration(
            Algorithm intAlgo,
            const std::shared_ptr<GaussianQuadrature> &gaussianQuadrature)
            : intAlgo_(intAlgo),
    ...
    Real AnalyticHestonEngine::Integration::calculate(
            Real c_inf,
            std::function<Real(Real)> f) const {
        Real retVal;
