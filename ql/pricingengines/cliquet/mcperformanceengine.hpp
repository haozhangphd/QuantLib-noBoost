...
/*
 Copyright (C) 2008 Master IMAFA - Polytech'Nice Sophia - UniversitГ© de Nice Sophia Antipolis
...
namespace QuantLib {
    ...
    inline TimeGrid MCPerformanceEngine<RNG,S>::timeGrid() const {
        ...
        for (Size i=0; i<arguments_.resetDates.size(); i++)
            fixingTimes.emplace_back(process_->time(arguments_.resetDates[i]));
        fixingTimes.emplace_back(process_->time(arguments_.exercise->lastDate()));
