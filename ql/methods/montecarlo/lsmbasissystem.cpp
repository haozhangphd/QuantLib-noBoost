...
namespace QuantLib {
    namespace {
        ...
        typedef std::vector<std::function<Real(const Real&)> > VF_R;
        typedef std::vector<std::function<Real(const Array&)> > VF_A;
        ...
        for (Size i = 0; i <= order; ++i) {
            switch (polyType) {
                case Monomial:
                    ...
                    break;
                case Laguerre:
                    ret[i] = [i](Real x) { return GaussLaguerrePolynomial().weightedValue(i, x); };
                    break;
                case Hermite:
                    ret[i] = [i](Real x) { return GaussHermitePolynomial().weightedValue(i, x); };
                    break;
                case Hyperbolic:
                    ret[i] = [i](Real x) { return GaussHyperbolicPolynomial().weightedValue(i, x); };
                    break;
                case Legendre:
                    ret[i] = [i](Real x) { return GaussLegendrePolynomial().weightedValue(i, x); };
                    break;
                case Chebyshev:
                    ret[i] = [i](Real x) { return GaussChebyshevPolynomial().weightedValue(i, x); };
                    break;
                case Chebyshev2nd:
                    ret[i] = [i](Real x) { return GaussChebyshev2ndPolynomial().weightedValue(i, x); };
                    break;
                default:
