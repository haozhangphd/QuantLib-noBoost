...
namespace QuantLib {
    ...
    class LsmBasisSystem {
      public:
        ...
        static std::vector<std::function<Real(const Real&)>> pathBasisSystem(Size order, PolynomType polyType);

        static std::vector<std::function<Real(const Array&)>> multiPathBasisSystem(Size dim, Size order, PolynomType polyType);
    };
