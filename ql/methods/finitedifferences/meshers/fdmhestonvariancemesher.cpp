...
namespace QuantLib {
    ...
    FdmHestonVarianceMesher::FdmHestonVarianceMesher(
        ...
        try {
            ...
            for (Size l=1; l<=tAvgSteps; ++l) {
                ...
                for (Size i=1; i < size; ++i) {
                    ...
                    p += ps;
                    const Real tmp = k*InverseCumulativeNonCentralChiSquare(
                        df, ncp, 1000, 1e-8)(p);

                    const Real vx = std::max(vTmp+minVStep, tmp);
                    p = CumulativeNonCentralChiSquareDistribution(df, ncp)(vx/k);
                    vTmp=vx;
        ...
        volaEstimate_ = GaussLobattoIntegral(100000, 1e-4)(
                    [&pGrid, &vGrid](Real x){return std::sqrt(LinearInterpolation(pGrid.begin(), pGrid.end(),vGrid.begin())(x));},
