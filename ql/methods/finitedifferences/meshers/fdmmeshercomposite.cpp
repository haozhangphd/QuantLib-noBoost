...
namespace QuantLib {
    ...
    namespace {
        ...
        std::vector<T> build_vector(const T& m1, const T& m2) {
            const std::vector<std::shared_ptr<Fdm1dMesher> > retVal = {m1, m2};
            return retVal;
        }

        std::vector<T> build_vector(const T& m1, const T& m2, const T& m3) {
            const std::vector<std::shared_ptr<Fdm1dMesher> > retVal = {m1, m2, m3};
            return retVal;
        }

        std::vector<T> build_vector(const T& m1, const T& m2,
                                    const T& m3, const T& m4) {
            const std::vector<std::shared_ptr<Fdm1dMesher> > retVal = {m1, m2, m3, m4};
            return retVal;
        }
