...
namespace QuantLib {
    ...
    namespace {
        ...
        Real int_ph(const HestonProcess& process,
            ...
            return gaussLaguerreIntegration(
                    [&process,y, nu_0, nu_t, t](Real s){return ph(process, y, s, nu_0, nu_t, t);})
                / std::sqrt(2*M_PI*(1-rho*rho)*y)
                * std::exp(-0.5*square(  x - x0 - a
                ...
                return (x < upper)
                    ? std::max(0.0, std::min(1.0,
                        gaussLaguerreIntegration(
                                [&process,x,nu_0,nu_t,dt](Real r){return ch(process,x, r, nu_0, nu_t, dt);})))
                    : 1.0;
              }
    ...
    Real HestonProcess::pdf(Real x, Real v, Time t, Real eps) const {
         ...
         return SegmentIntegral(100)(
                 [this, a, x,  v, t](Real r){return int_ph(*this, a, x, r, this->v0_, v, t);},
               QL_EPSILON, upper)
        ...
        switch (discretization_) {
          ...
          case BroadieKayaExactSchemeTrapezoidal:
          {
            ...
            const Real x = std::min(1.0-QL_EPSILON,
                std::max(0.0, Real(CumulativeNormalDistribution()(dw[2]))));

            const Real vds = Brent().solve(
                    [this, nu_0, nu_t, dt, x](Real r){return cdf_nu_ds_minus_x(*this, r, nu_0, nu_t, dt, this->discretization_, x);},
    ...
    Real HestonProcess::varianceDistribution(Real v, Real dw, Time dt) const {
        ...
        const Real p = std::min(1.0-QL_EPSILON,
            std::max(0.0, Real(CumulativeNormalDistribution()(dw))));

        return sigma_*sigma_*(1-std::exp(-kappa_*dt))/(4*kappa_)
            *InverseCumulativeNonCentralChiSquare(df, ncp, 100)(p);
    }
}
