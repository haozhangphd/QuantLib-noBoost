        ...
        class VannaVolgaInterpolationImpl
          ...
          public:
            ...
            void update() {
                ...
                for(Size i = 0; i < 3; i++){
                    premiaBS.emplace_back(blackFormula(Option::Call, this->xBegin_[i], fwd_, atmVol_ * std::sqrt(T_), dDiscount_));
                    premiaMKT.emplace_back(blackFormula(Option::Call, this->xBegin_[i], fwd_, this->yBegin_[i] * std::sqrt(T_), dDiscount_));
                    vegas.emplace_back(vega(this->xBegin_[i]));
                }
            }
