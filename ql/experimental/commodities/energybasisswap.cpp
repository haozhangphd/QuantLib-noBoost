...
namespace QuantLib {
    ...
    EnergyBasisSwap::EnergyBasisSwap(
                    ...
                    const std::shared_ptr<CommodityIndex>& spreadIndex,
                    const std::shared_ptr<CommodityIndex>& payIndex,
                    const std::shared_ptr<CommodityIndex>& receiveIndex,
                    bool spreadToPayLeg,
    ...
    void EnergyBasisSwap::performCalculations() const {
        ...
        try {
            ...
            for (PricingPeriods::const_iterator pi = pricingPeriods_.begin();
                ...
                const std::shared_ptr<PricingPeriod>& pricingPeriod = *pi;
                ...
                paymentCashFlows_[pricingPeriod->paymentDate()] =
                    std::make_shared<CommodityCashFlow>(pricingPeriod->paymentDate(),
                                                 Money(baseCurrency,
