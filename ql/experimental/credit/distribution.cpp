...
//     }
    ...
    //-------------------------------------------------------------------------
        ...
        density_.emplace_back((cumulativeDensity_[0]-0.)/dx_[0]);
        excessProbability_.emplace_back(1.);
        for(Integer i=1; i<size_-1; i++) {
            excessProbability_.emplace_back(1.-cumulativeDensity_[i-1]);
            density_.emplace_back((cumulativeDensity_[i]-
                cumulativeDensity_[i-1])/dx_[i]);
        }
        excessProbability_.emplace_back(1.-cumulativeDensity_.back());
        density_.emplace_back((1.-cumulativeDensity_.back())/dx_.back());
    }
