...
namespace QuantLib {
    ...
    */
    ...
    inline void SpotRecoveryLatentModel<CP>::resetBasket(
        const std::shared_ptr<Basket> basket) const
    {
        basket_ = basket;
        ...
        Size iName) const 
    {
        ...
        return integratedExpectedValue(
                [this, invP, invRR, iName](const std::vector<Real>& v){return this->conditionalExpLossRRInv(invP, invRR, iName, v);});
    }
            ...
            */
        {
            ...
            for(Size iB=0; iB<factorWeights[iName].size(); iB++)
                ...
                cumul += factorWeights[iName][iB] * 
                    ...
                    factorWeights[iName + numNames_][iB];
            crossIdiosyncFctrs_.emplace_back(cumul);
        }
