    ...
    */
    ...
    std::vector<Size>  
        ...
        for (Size i = 0; i < size(); i++)
            if (!pool_->get(pool_->names()[i]).defaultedBetween(
                    ...
                    endDate,
                    pool_->defaultKeys()[i]))
                calcBufferLiveList.emplace_back(i);
        ...
        Basket::remainingNotionals(const Date& endDate) const 
    {
        ...
        for(Size i=0; i<alive.size(); i++)
            calcBufferNotionals.emplace_back(
        ...
        Basket::remainingProbabilities(const Date& d) const 
    {
        ...
        for(Size i=0; i<alive.size(); i++)
            prob.emplace_back(pool_->get(pool_->names()[i]).defaultProbability(
        ...
        Basket::remainingNames(const Date& endDate) const 
    {
        ...
        for(Size i=0; i<alive.size(); i++)
            calcBufferNames.emplace_back(pool_->names()[alive[i]]);
        ...
        Basket::remainingDefaultKeys(const Date& endDate) const 
    {
        ...
        for(Size i=0; i<alive.size(); i++)
            defKeys.emplace_back(pool_->defaultKeys()[alive[i]]);
        return defKeys;
    }
