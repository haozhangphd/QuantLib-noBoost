...
// nope! use max common divisor. See O'Kane. Or give both options at least.
      ...
      private:
          ...
          */
        std::map<Real, Probability> conditionalLossDistrib(
        ...
        Real expectedConditionalLoss(const std::vector<Probability>& pDefDate, //<< never used!!
            const std::vector<Real>& mktFactor) const;
        std::vector<Real> conditionalLossProb(
            const std::vector<Probability>& pDefDate, 
            //const Date& date,
            const std::vector<Real>& mktFactor) const;
        //versions using the P-inverse, deprecate the former
        std::map<Real, Probability> conditionalLossDistribInvP(
...
/*
    ...
    ///////////////////////////////////////////////////////////////////////
        ...
        return copula_->integratedExpectedValue(
            [this, &uncDefProb](const std::vector<Real>& x){return expectedConditionalLoss(uncDefProb, x);}
            );
            */
/**/
        std::vector<Probability> uncDefProb =
        ...
        for (Size i = 0; i < uncDefProb.size(); ++i)
            invProb.emplace_back(copula_->inverseCumulativeY(uncDefProb[i], i));
        ///  invProb.emplace_back(CP::inverseCumulativeY(uncDefProb[i], i));//<-static call
        return copula_->integratedExpectedValue(
                [this, &invProb](const std::vector<Real> &x) { return expectedConditionalLossInvP(invProb, x); }
        );
    ...
    inline std::vector<Real> 
        ...
        std::vector<Probability> uncDefProb = 
            basket_->remainingProbabilities(date);
        return copula_->integratedExpectedValue(
            [this, &uncDefProb](const std::vector<Real>& x){return conditionalLossProb(uncDefProb, x);}
            );
    }
    ...
    void RecursiveLossModel<CP>::resetModel() {
        ...
        for(Size i=0; i<remainingBsktSize_; ++i)
            lgds.emplace_back(notionals_[i]*(1.-copula_->recoveries()[i]));
        lgdsTmp = lgds;
        ...
        lossUnit_ = *(std::min_element(lgds.begin(), lgds.end()))
            / nBuckets_;
        for(Size i=0; i<remainingBsktSize_; ++i)
            wk_.emplace_back(std::floor(lgdsTmp[i]/lossUnit_ + .5));
    }
   ...
   //         loss = std::max(std::min(loss, detachAmount_)-attachAmount_, 0.);
        ...
        const std::vector<Real>& mktFactor) const 
    {
        ...
        while(distIt != pIndepDistrib.end()) {
            ...
            results.emplace_back(distIt->second);
             ++distIt;
        }
        return results;
    }
