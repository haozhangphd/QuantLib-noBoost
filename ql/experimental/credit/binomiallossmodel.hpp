        ...
        */
        Disposable<std::vector<Real>>
            ...
            return copula_->integratedExpectedValue(
			  [this, date, notionals, invProbs] (const std::vector<Real>& v) ->Disposable<std::vector<Real>> {return lossProbability(std::cref(date), std::cref(notionals), std::cref(invProbs), v);}
                );
        }
    ...
    protected:
                               ...
                               const std::vector<Real>& mktFactors) const
        {
            ...
            for(Size i=0; i<evalDateLives.size(); i++) 
                condLgds.emplace_back(1.-copula_->conditionalRecovery(d, 
    ...
    */
        ...
        const std::vector<Real>& mktFctrs) const 
    {
        ...
        */
        ...
        std::transform(fractionalEL.begin(), fractionalEL.end(), 
            ...
            [notBskt] (Real i1, Real i2){return i1 * i2 / notBskt;});
        ...
        BinomialLossModel<LLM>::lossPoints(const Date& d) const 
    {
        ...
        for(Size i=0; i<dataSize; i++)
            data.emplace_back(i * aveLossFrct * outsNot);
        return data;
    }
    ...
    Real BinomialLossModel<LLM>::expectedTrancheLoss(const Date& d) const {
        ...
        return copula_->integratedExpectedValue(
		    [this, &d, &lossVals, &notionals, &invProbs](const std::vector<Real>& v) {return this->condTrancheLoss(d, lossVals, notionals, invProbs, v);}
            );
    }
