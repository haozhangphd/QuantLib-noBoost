    ...
    */
    ...
    class InterpolatedYoYCapFloorTermPriceSurface
      ...
      public:
        ...
        virtual std::shared_ptr<YoYInflationTermStructure>
    ...
    intersect() const {
        ...
        for (Size i=0; i<cfMaturities_.size();i++) {
            cfMaturityTimes_.emplace_back(timeFromReference(
        ...
        for (Size i = 0; i < cfMaturities_.size(); i++) {
            ...
            if (trialsExceeded == false) {
                ...
                {
                    tmpSwapMaturities.emplace_back(t);
                    tmpSwapRates.emplace_back(kI);
        ...
        for (Size i=0; i<cfMaturities_.size(); ++i) {
            if ( !validMaturity[i] ) {
                atmYoYSwapDateRates_.first.emplace_back(referenceDate()+cfMaturities_[i]);
                atmYoYSwapTimeRates_.first.emplace_back(timeFromReference(referenceDate()+cfMaturities_[i]));
                // atmYoYSwapRates_->second.emplace_back(interpol((*cfMaturities_)[i]));
                ...
                atmYoYSwapTimeRates_.second.emplace_back(newSwapRate);
                atmYoYSwapDateRates_.second.emplace_back(newSwapRate);
            } else {
                atmYoYSwapTimeRates_.first.emplace_back(tmpSwapMaturities[counter]);
                atmYoYSwapTimeRates_.second.emplace_back(tmpSwapRates[counter]);
                atmYoYSwapDateRates_.first.emplace_back(
                    yoyOptionDateFromTenor(cfMaturities_.at(counter)));
                atmYoYSwapDateRates_.second.emplace_back(tmpSwapRates[counter]);
                counter++;
            }
        }
