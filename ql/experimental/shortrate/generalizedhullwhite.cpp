    ...
    */
    ...
    : OneFactorAffineModel(2), TermStructureConsistentModel(yieldtermStructure),
      ...
      f_(f), fInverse_(fInverse) {
        ...
        speedperiods_.emplace_back(0.0);
        for (Size i=0;i<speedstructure.size()-1;i++)
            speedperiods_.emplace_back(dc.yearFraction(speedstructure[0],
                                                    speedstructure[i+1]));

        a_ = PiecewiseConstantParameter2(speedperiods_, PositiveConstraint());

        volperiods_.emplace_back(0.0);
        for (Size i=0;i<volstructure.size()-1;i++)
            volperiods_.emplace_back(dc.yearFraction(volstructure[0],
                                                  volstructure[i+1]));
    ...
    : OneFactorAffineModel(2), TermStructureConsistentModel(yieldtermStructure),
      ...
      f_(f), fInverse_(fInverse) {
        ...
        for (Size i=0;i<speedstructure.size();i++)
            speedperiods_.emplace_back(dc.yearFraction(ref,
                                                    speedstructure[i]));

        a_ = PiecewiseConstantParameter2(speedperiods_, PositiveConstraint());

        for (Size i=0;i<volstructure.size();i++)
            volperiods_.emplace_back(dc.yearFraction(ref,
                                                  volstructure[i]));
    ...
    : OneFactorAffineModel(2),
      ...
      sigma_(arguments_[1]) {
        ...
        speedperiods_.emplace_back(0.0);
        volperiods_.emplace_back(0.0);
        speedstructure_.emplace_back(ref);
        volstructure_.emplace_back(ref);
    ...
    std::function<Real (Time)> GeneralizedHullWhite::speed() const {
        ...
        speedvals.emplace_back(a_(0.001));

        if (a_.size()==1) {
            std::vector<Time> speedper;
            speedper.emplace_back(0.0);
            speedper.emplace_back(200); // 200 years to match the
    ...
    std::function<Real (Time)> GeneralizedHullWhite::vol() const {
        ...
        volvals.emplace_back(sigma_(0.001));

        if (sigma_.size()==1) {
            std::vector<Time> volper;
            volper.emplace_back(0.0);
            volper.emplace_back(200); //200 years to approximate the
