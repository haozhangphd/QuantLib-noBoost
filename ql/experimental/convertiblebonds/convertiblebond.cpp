...
namespace QuantLib {
    ...
    ConvertibleBond::option::option(
            ...
            Natural settlementDays,
            Real redemption)
    : OneAssetOption(std::shared_ptr<StrikedTypePayoff>(new
      ...
      settlementDays_(settlementDays), redemption_(redemption) {
        registerWith(std::shared_ptr<ConvertibleBond>(const_cast<ConvertibleBond*>(bond),
                                                        null_deleter));
    }
    ...
    void ConvertibleBond::option::setupArguments(
        ...
        for (Size i=0; i<n; i++) {
            if (!callability_[i]->hasOccurred(settlement, false)) {
                moreArgs->callabilityTypes.emplace_back(callability_[i]->type());
                moreArgs->callabilityDates.emplace_back(callability_[i]->date());
                moreArgs->callabilityPrices.emplace_back(
                ...
                std::shared_ptr<SoftCallability> softCall =
                    std::dynamic_pointer_cast<SoftCallability>(
                                                             callability_[i]);
                if (softCall)
                    moreArgs->callabilityTriggers.emplace_back(
                                                         softCall->trigger());
                else
                    moreArgs->callabilityTriggers.emplace_back(Null<Real>());
            }
        }
        ...
        for (Size i=0; i<cashflows.size()-1; i++) {
            if (!cashflows[i]->hasOccurred(settlement, false)) {
                moreArgs->couponDates.emplace_back(cashflows[i]->date());
                moreArgs->couponAmounts.emplace_back(cashflows[i]->amount());
            }
        }
        ...
        for (Size i=0; i<dividends_.size(); i++) {
            if (!dividends_[i]->hasOccurred(settlement, false)) {
                moreArgs->dividends.emplace_back(dividends_[i]);
                moreArgs->dividendDates.emplace_back(dividends_[i]->date());
            }
        }
