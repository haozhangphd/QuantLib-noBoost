...
namespace QuantLib {
      ...
      a_(0.)
    {
        ...
        for(Size i=0; i<degreesFreedom_.size(); i++)
            polynCharFnc_.emplace_back(polynCharactT((degreesFreedom[i]-1)/2));
    ...
    CumulativeBehrensFisher::polynCharactT(Natural n) const {
        ...
        high.emplace_back(std::sqrt(static_cast<Real>(nu)));
        if(n==0) return low;
        if(n==1) return high;

        for(Size k=1; k<n; k++) {
            std::vector<Real> recursionFactor(1,0.); // 0 coef
            recursionFactor.emplace_back(0.); // 1 coef
            recursionFactor.emplace_back(nu/((2.*k+1.)*(2.*k-1.))); // 2 coef
