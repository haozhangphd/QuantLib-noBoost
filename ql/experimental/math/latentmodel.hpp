...
namespace QuantLib {
    ...
    namespace detail {
        ...
        struct multiplyV {
            ...
            std::vector<Real>
            operator()(Real d, std::vector<Real>&& v) {
    ...
    */
    ...
    public:
        ...
        virtual Real integrate(std::function<Real(
    ...
    public:
        ...
        Real integrate(std::function<Real(
                const std::vector<Real> &arg)> f) const {
    ...
    */
    ...
    class LatentModel
            : public virtual Observer, public virtual Observable {//observer if factors as quotes
    public:
        void update();
    ...
    public:
        ...
        */
        Real integratedExpectedValue(
                std::function<Real(const std::vector<Real> &)> f) const {
            // function composition: composes the integrand with the density 
            //   through a product.
            return
                    integration()->integrate(
                            [this, &f](const std::vector<Real> &v) {
                                return (this->copula_.density(v) * f(v));
                            });
        }
        ...
        /*! Integrates an arbitrary vector function over the density domain(i.e.
         computes its expected value).
        */
        std::vector<Real> integratedExpectedValue(
            ...
            return
                    integration()->integrateV(//see note in LMIntegrators base class
                            [this, &f](const std::vector<Real> &v) -> std::vector<Real> {
                                return detail::multiplyV()(this->copula_.density(v), f(v));
                            });
        }
    ...
    LatentModel<Impl>::LatentModel(
        ...
        for (Size i = 0; i < factorWeights.size(); i++) {
            idiosyncFctrs_.emplace_back(std::sqrt(1. -
    ...
    LatentModel<Impl>::LatentModel(
        ...
        for (Size iName = 0; iName < factorWeights.size(); iName++)
            factorWeights_.emplace_back(std::vector<Real>(1,
                                                          factorWeights[iName]));
        for (Size iName = 0; iName < factorWeights.size(); iName++)
            idiosyncFctrs_.emplace_back(std::sqrt(1. -
