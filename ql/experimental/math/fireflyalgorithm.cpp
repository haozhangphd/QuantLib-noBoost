...
namespace QuantLib {
    ...
    void FireflyAlgorithm::startState(Problem &P, const EndCriteria &endCriteria) {
        ...
        for (Size i = 0; i < M_; i++) {
            ...
            x_.emplace_back(Array(N_, 0.0));
            xI_.emplace_back(Array(N_, 0.0));
            xRW_.emplace_back(Array(N_, 0.0));
            Array& x = x_.back();
            for (Size j = 0; j < N_; j++) {
            ...
            //Evaluate point
            values_.emplace_back(std::make_pair(P.value(x), i));
        }
    ...
    EndCriteria::Type FireflyAlgorithm::minimize(Problem &P, const EndCriteria &endCriteria) {
        ...
        uniform_integer::param_type nParam(0, N_ );
