...
class PiecewiseIntegral : public Integrator {
  public:
    PiecewiseIntegral(const std::shared_ptr<Integrator> &integrator,
                      const std::vector<Real> &criticalPoints,
                      const bool avoidCriticalPoints = true);

  protected:
    Real integrate(std::function<Real(Real)>f, Real a, Real b) const;

  private:
    Real integrate_h(std::function<Real(Real)>f, Real a,
                     Real b) const;
...
inline Real PiecewiseIntegral::integrate_h(std::function<Real(Real)>f,
                                           Real a, Real b) const {
...
inline Real PiecewiseIntegral::integrate(std::function<Real(Real)>f,
                                         Real a, Real b) const {
