...
namespace QuantLib {
        ...
        const initTraits& vals)
    {
        for(Size iFactor=0; iFactor<vals.tOrders.size(); iFactor++) {
            ...
            distributions_.emplace_back(std::student_t_distribution<long double>(
            ...
            // numerical errors.
            varianceFactors_.emplace_back(std::sqrt(
        ...
        for(Size iLVar=0; iLVar<factorWeights.size(); iLVar++) {
            ...
            for(Size iFactor=0; iFactor<factorWeights[iLVar].size(); iFactor++)
                normFactorWeights.emplace_back(factorWeights[iLVar][iFactor] * 
                    varianceFactors_[iFactor]);
            // idiosincratic term, all Z factors are assumed identical.
            normFactorWeights.emplace_back(idiosyncFctr * varianceFactors_.back());
            latentVarsCumul_.emplace_back( 
                CumulativeBehrensFisher(vals.tOrders, normFactorWeights));
            latentVarsInverters_.emplace_back(
