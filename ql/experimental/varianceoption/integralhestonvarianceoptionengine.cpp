...
namespace QuantLib {
    ...
    {
        Real ss=0.0;
        std::vector<double> xiv(2048*2048+1);
        double nris=0.0;
        int j=0,mm=0;
        double pi=0,pi2=0;
        double dstep=0;
        ...
        std::vector<Complex> ff(2048*2048);
        Complex xi;
    ...
    Real IvopTwoDim(Real eps, Real chi, Real theta, Real /*rho*/,
                    ...
                    std::function<Real(Real)> payoff) {

        Real ss=0.0;
        std::vector<double> xiv(2048*2048+1);
        std::vector<double> ivet(2048 * 2048 + 1);
        double nris=0.0;
        int j=0,mm=0,k=0;
        double pi=0,pi2=0;
    ...
    struct payoff_adapter {
        ...
        payoff_adapter(std::shared_ptr<QuantLib::Payoff> payoff)
        : payoff(payoff) {}
