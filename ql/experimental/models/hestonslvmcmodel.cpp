...
namespace QuantLib {
    HestonSLVMCModel::HestonSLVMCModel(
        ...
        const std::shared_ptr<BrownianGeneratorFactory>& brownianGeneratorFactory,
        const Date& endDate,
    ...
    : localVol_(localVol),
      ...
      calibrationPaths_(calibrationPaths) {
        ...
        for (Size i=0; i < mandatoryDates.size(); ++i) {
            gridTimes.emplace_back(dc.yearFraction(refDate, mandatoryDates[i]));

        }
        gridTimes.emplace_back(dc.yearFraction(refDate, endDate));
    ...
    void HestonSLVMCModel::performCalculations() const {
        ...
        const std::shared_ptr<HestonSLVProcess> slvProcess
            = std::make_shared<HestonSLVProcess>(hestonProcess, leverageFunction_);
