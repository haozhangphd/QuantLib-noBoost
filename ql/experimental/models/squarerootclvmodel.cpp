...
namespace QuantLib {
    ...
    const {
        ...
        const Real xMin = std::max(x.front(),
            Real((pMin_ == Null<Real>())
                ? 0.0 : InverseCumulativeNonCentralChiSquare(p.first, p.second)(pMin_)));

        const Real xMax = std::min(x.back(),
            Real((pMax_ == Null<Real>())
            ? QL_MAX_REAL : InverseCumulativeNonCentralChiSquare(p.first, p.second)(pMax_)));
	...
	Array s(x.size());
        for (Size i=0, n=s.size(); i < n; ++i) {
            const Real q = CumulativeNonCentralChiSquareDistribution(params.first, params.second)(x[i]);
