...
namespace QuantLib {
    ...
    : BlackAtmVolCurve(settlDays, cal, bdc, dc),
      ...
      interpolation_(std::shared_ptr<AbcdInterpolation>()) // do not initialize with nOptionTenors_
    {
        checkInputs();
    ...
    void AbcdAtmVolCurve::initializeOptionDatesAndTimes() const
    {
        ...
        for (Size i=0; i<nOptionTenors_; ++i) {
            if(inclusionInInterpolation_[i]==true) {
               actualOptionTimes_.emplace_back(optionTimes_[i]);
               actualOptionTenors_.emplace_back(optionTenors_[i]);
            }
        }
    }
    ...
    void AbcdAtmVolCurve::initializeVolatilities() {
        ...
        for (Size i=0; i<nOptionTenors_; ++i) {
            vols_[i] = volHandles_[i]->value();
            if(inclusionInInterpolation_[i]==true)
               actualVols_.emplace_back(vols_[i]);
        }
    }
