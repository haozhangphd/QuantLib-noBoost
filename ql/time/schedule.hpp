...
namespace QuantLib {
    ...
    */
    ...
    Schedule::terminationDateBusinessDayConvention() const {
        QL_REQUIRE(terminationDateConvention_ != std::nullopt,
    ...
    inline DateGeneration::Rule Schedule::rule() const {
        QL_REQUIRE(rule_ != std::nullopt, "full interface (rule) not available");
        return *rule_;
    }

    inline bool Schedule::endOfMonth() const {
        QL_REQUIRE(endOfMonth_ != std::nullopt,
