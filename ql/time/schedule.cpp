...
namespace QuantLib {
      ...
      nextToLastDate_(nextToLast==terminationDate ? Date() : nextToLast)
    {
        ...
        switch (*rule_) {
          ...
          case DateGeneration::Zero:
            tenor_ = 0*Years;
            dates_.emplace_back(effectiveDate);
            dates_.emplace_back(terminationDate);
            isRegular_.emplace_back(true);
            break;
          ...
          case DateGeneration::Forward:
            ...
            if (*rule_ == DateGeneration::CDS || *rule_ == DateGeneration::CDS2015) {
                dates_.emplace_back(previousTwentieth(effectiveDate, *rule_));
            } else {
                dates_.emplace_back(effectiveDate);
            }
            ...
            for (;;) {
                ...
                if (temp > exitDate) {
                    if (nextToLastDate_ != Date() &&
                        ...
                        dates_.emplace_back(nextToLastDate_);
                        isRegular_.emplace_back(false);
                    }
                    break;
                } else {
            ...
            if (calendar_.adjust(dates_.back(),terminationDateConvention)!=
                ...
                if (*rule_ == DateGeneration::Twentieth ||
                    ...
                    *rule_ == DateGeneration::OldCDS ||
                    *rule_ == DateGeneration::CDS) {
                    dates_.emplace_back(nextTwentieth(terminationDate, *rule_));
                    isRegular_.emplace_back(true);
                    ...
                    if(tentativeTerminationDate.month() %2 == 0) {
                        dates_.emplace_back(tentativeTerminationDate);
                        isRegular_.emplace_back(true);
                    }
                } else {
                    dates_.emplace_back(terminationDate);
    ...
    Schedule Schedule::until(const Date& truncationDate) const {
        ...
        if (truncationDate<result.dates_.back()) {
            ...
            if (truncationDate!=result.dates_.back()) {
                result.dates_.emplace_back(truncationDate);
                result.isRegular_.emplace_back(false);
