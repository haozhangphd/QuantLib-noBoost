...
namespace QuantLib {
    ...
    */
    class CumulativePoissonDistribution {
      public:
        CumulativePoissonDistribution(long double mu) : mu_(mu) {}
        long double operator()(BigNatural k) const {
    ...
    */
    class InverseCumulativePoisson {
      public:
        InverseCumulativePoisson(long double lambda = 1.0);
        long double operator()(long double x) const;
      private:
        long double lambda_;
        long double calcSummand(BigNatural index) const;
    };
    ...
    inline PoissonDistribution::PoissonDistribution(long double mu)
    : mu_(mu) {
    ...
    inline long double PoissonDistribution::operator()(BigNatural k) const {
        if (mu_==0.0) {
            if (k==0) return 1.0;
            else      return 0.0;
        }
        long double logFactorial = Factorial::ln(k);
        return std::exp(k*std::log(mu_) - logFactorial - mu_);
    }


    inline InverseCumulativePoisson::InverseCumulativePoisson(long double lambda)
    : lambda_(lambda) {
        QL_REQUIRE(lambda_ > 0.0, "lambda must be positive");
    }

    inline long double InverseCumulativePoisson::operator()(long double x) const {
    ...
    inline long double InverseCumulativePoisson::calcSummand(BigNatural index) const {
