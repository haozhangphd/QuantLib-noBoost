...
namespace QuantLib {
    ...
    Disposable<std::vector<Size> > qrDecomposition(const Matrix& M,
        ...
        std::vector<int> lipvt(n);
        std::vector<Real> rdiag(n);
        std::vector<Real> wa(n);

        MINPACK::qrfac(m, n, &(*mT.begin()), 0, (pivot)?1:0,
                       lipvt.data(), n, rdiag.data(), rdiag.data(), wa.data());
        ...
        if (pivot) {
            std::copy(lipvt.begin(), lipvt.begin() + n, ipvt.begin());
        }
        else {
            for (Size i=0; i < n; ++i)
                ipvt[i] = i;
    ...
    Disposable<Array> qrSolve(const Matrix& a, const Array& b,
        ...
        std::copy(lipvt.begin(), lipvt.end(), ipvt.begin());

        Matrix rT = transpose(r);

        std::vector<Real> sdiag(n);
        std::vector<Real> wa(n);
        ...
        MINPACK::qrsolv(n, &(*rT.begin()), n, ipvt.data(),
                        &(*ld.begin()), &(*qtb.begin()),
                        &(*x.begin()), sdiag.data(), wa.data());
