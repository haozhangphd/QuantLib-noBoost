...
namespace QuantLib {
    ...
    public:
        ...
        SparseMatrixGeneral(int row_size, int column_size, size_t non_zeroes = 0)
        ...
        SparseMatrixGeneral(int row_size, int column_size, std::vector<T> &&values, std::vector<int> &&columns,
                            std::vector<int> &&rowIndex, size_t filled_row_until)
        ...
        SparseMatrixGeneral(int row_size, int column_size, const std::vector<T> &values,
                            ...
                            const std::vector<int> &rowIndex, size_t filled_row_until) = delete;

        //matrix info
        size_t filled_size() const { return values_.size(); }
        ...
        //element access
        element_proxy<T> operator()(size_t m, size_t n) {
            return element_proxy<T>(m, n, this);
        }

        const T operator()(size_t m, size_t n) const {
            std::pair<int, size_t> index = findIndex(m, n);
    ...
    private:
        ...
        const std::pair<int, size_t> findIndex(size_t row, size_t column) const;

        void insert(size_t index, size_t row, size_t column, const T &x);
    ...
    Array SparseMatrixGeneral<T>::operator*(const Array &x) const {
        QL_REQUIRE(static_cast<int>(x.size()) == column_size_,
...
#else
        Array ret(row_size_);
        for (size_t i = 0; i < filled_row_until_ - 1; ++i) {
            Real sum = 0.0;
...
//Matrix-matrix operations
    ...
    SparseMatrixGeneral<T> &SparseMatrixGeneral<T>::operator+=(const SparseMatrixGeneral<T> &x) {
...
#ifdef QL_USE_MKL
        const char transa = 'N';
        const size_t request = 0;
        const size_t sort = 0;
        const double beta = 1;
        const size_t nzmax = column_size_ * row_size_;

        std::vector<size_t> rowIndex_copy(rowIndex_);
        std::vector<size_t> rowIndex_copy_other(x.rowIndex_);

        size_t temp1 = rowIndex_[filled_row_until_ - 1];
        std::for_each(rowIndex_copy.begin() + filled_row_until_, rowIndex_copy.end(), [&temp1](size_t &a) { a = temp1; });
        size_t temp2 = x.rowIndex_[x.filled_row_until_ - 1];
        std::for_each(rowIndex_copy_other.begin() + x.filled_row_until_, rowIndex_copy_other.end(), [&temp2](size_t &a) { a = temp2; });

        //return values
        std::vector<T> c(nzmax);
        std::vector<size_t> jc(nzmax);
        std::vector<size_t> ic(row_size_+1);
        size_t info;
        ...
        c.shrink_to_fit();
        std::vector<size_t>::iterator last = std::find(ic.begin(), ic.end(), jc.size() + 1);
        ...
        std::vector<size_t> columns_ret;
        columns_ret.reserve(columns_.size());
        size_t row_end = std::min(filled_row_until_, x.filled_row_until_) - 1;
        ...
        for (size_t row = 0; row < row_end; ++row) {
            size_t column_left = rowIndex_[row] - 1;
            size_t column_right = x.rowIndex_[row] - 1;
            size_t column_left_end = rowIndex_[row + 1] - 1;
            size_t column_right_end = x.rowIndex_[row + 1] - 1;
            ...
            if (column_left < column_left_end) {
                size_t size = values_ret.size() + column_left_end - column_left;
                ...
                size_t size = values_ret.size() + column_right_end - column_right;
                values_ret.reserve(size);
                columns_ret.reserve(size);
        ...
        if (filled_row_until_ > x.filled_row_until_) {
            ...
            columns_ret.insert(columns_ret.end(), columns_.begin() + rowIndex_[x.filled_row_until_] - 1,
                               columns_.end());
            size_t new_elements = rowIndex_[x.filled_row_until_ - 1] - original_elements;
            std::for_each(rowIndex_.begin() + x.filled_row_until_, rowIndex_.begin() + filled_row_until_,
                          [&new_elements](size_t &s) { s += new_elements; });
        } else if (filled_row_until_ < x.filled_row_until_) {
            ...
            columns_ret.insert(columns_ret.end(), x.columns_.begin() + x.rowIndex_[filled_row_until_] - 1,
                               x.columns_.end());
            size_t new_elements = rowIndex_[filled_row_until_ - 1] - original_elements;
            std::transform(x.rowIndex_.begin() + filled_row_until_, x.rowIndex_.begin() + x.filled_row_until_,
                           rowIndex_.begin() + filled_row_until_, [&new_elements](size_t s) { return s + new_elements; });
    ...
    SparseMatrixGeneral<T> SparseMatrixGeneral<T>::operator+(const SparseMatrixGeneral<T> &x) const {
        ...
        size_t filled_row_until = std::distance(ic.begin(), last) + 1;
        ...
        size_t filled_row_until_ret;
        if (filled_row_until_ > x.filled_row_until_) {
            values_ret.insert(values_ret.end(), values_.begin() + rowIndex_[x.filled_row_until_] - 1, values_.end());
            columns_ret.insert(columns_ret.end(), columns_.begin() + rowIndex_[x.filled_row_until_] - 1,
                               columns_.end());
            size_t new_elements = rowIndex_ret[x.filled_row_until_ - 1] - original_elements;
            std::transform(rowIndex_.begin() + x.filled_row_until_, rowIndex_.begin() + filled_row_until_,
                           std::back_inserter(rowIndex_ret), [&new_elements](size_t s) { return s + new_elements; });
    ...
    SparseMatrixGeneral<T> &SparseMatrixGeneral<T>::operator-=(const SparseMatrixGeneral<T> &x) {
        ...
        size_t temp2 = x.rowIndex_[x.filled_row_until_ - 1];
        std::for_each(rowIndex_copy_other.begin() + x.filled_row_until_, rowIndex_copy_other.end(), [&temp2](size_t &a) { a = temp2; });
        std::for_each(values_copy_other.begin(), values_copy_other.end(), [](T& s) {  s = -s; });
        ...
        if (filled_row_until_ > x.filled_row_until_) {
            ...
            std::transform(x.values_.begin() + x.rowIndex_[filled_row_until_] - 1, x.values_.end(),
                           std::back_inserter(values_ret), [](Real &s) { return -s; });
            columns_ret.insert(columns_ret.end(), x.columns_.begin() + x.rowIndex_[filled_row_until_] - 1,
                               x.columns_.end());
            size_t new_elements = rowIndex_[filled_row_until_ - 1] - original_elements;
            std::transform(x.rowIndex_.begin() + filled_row_until_, x.rowIndex_.begin() + x.filled_row_until_,
    ...
    SparseMatrixGeneral<T> SparseMatrixGeneral<T>::operator-(const SparseMatrixGeneral<T> &x) const {
        ...
        //since sort is disabled, the const variables are not modified
        mkl_dcsradd(&transa, &request, &sort, &row_size_, &column_size_, const_cast<T *>(values_.data()),
                    const_cast<size_t *>(columns_.data()), rowIndex_copy.data(), &beta, values_copy_other.data()),
                    const_cast<size_t *>(x.columns_.data(), rowIndex_copy_other.data(), c.data(), jc.data(), ic.data(), &nzmax, &info);
        QL_REQUIRE(info == 0, "Matrix addition cannot be performed.");
        ...
        std::vector<size_t> columns_ret;
        columns_ret.reserve(columns_.size());
        std::vector<size_t> rowIndex_ret;
        rowIndex_ret.reserve(row_size_ + 1);
        rowIndex_ret.emplace_back(1);
        size_t row_end = std::min(filled_row_until_, x.filled_row_until_) - 1;
        size_t original_elements;
        ...
        if (filled_row_until_ > x.filled_row_until_) {
            ...
            columns_ret.insert(columns_ret.end(), x.columns_.begin() + x.rowIndex_[filled_row_until_] - 1,
                               x.columns_.end());
            size_t new_elements = rowIndex_ret[filled_row_until_ - 1] - original_elements;
            std::transform(x.rowIndex_.begin() + filled_row_until_, x.rowIndex_.begin() + x.filled_row_until_,
                           std::back_inserter(rowIndex_ret), [&new_elements](size_t s) { return s + new_elements; });
            filled_row_until_ret = x.filled_row_until_;
        } else
    ...
    template<typename T>
    const std::pair<int, size_t> SparseMatrixGeneral<T>::findIndex(size_t row, size_t column) const {
        // in a non-filled row
        ...
        auto index = std::equal_range(columns_.begin() + rowIndex_[row] - 1, columns_.begin() + rowIndex_[row + 1] - 1,
                                      column + 1);
        size_t i = std::distance(columns_.begin(), index.first);
    ...
    template<typename T>
    void SparseMatrixGeneral<T>::insert(size_t index, size_t row, size_t column, const T &x) {
        columns_.insert(columns_.begin() + index, column + 1);
        values_.insert(values_.begin() + index, x);

        //if brand new row
        if (row + 1 >= filled_row_until_) {
            size_t temp = rowIndex_[filled_row_until_ - 1];
    ...
    class element_proxy {
    public:
        element_proxy(size_t m, size_t n, SparseMatrixGeneral<T> *parent) : m_(m), n_(n), index_(parent->findIndex(m, n)),
                                                                      parent_(parent) {}
    ...
    private:
        const size_t m_;
        const size_t n_;
        const std::pair<int, size_t> index_;
    ...
    public:
        ...
        element_proxy<Real> operator()(size_t i, size_t j) {
            return data_(i, j);
        }
