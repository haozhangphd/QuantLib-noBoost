    ...
    */
    ...
    Real GenericRiskStatistics<S>::regret(Real target) const {
        ...
        std::pair<Real,Size> result =
            this->expectationValue([target](Real x){return (x-target)*(x-target);},
                                   [target](Real x){return x<target;});
        Real x = result.first;
    ...
    Real GenericRiskStatistics<S>::expectedShortfall(Real centile) const {
        ...
        std::pair<Real,Size> result =
            this->expectationValue([](Real x) {return x;}, [target](Real x){return x < target;});
        Real x = result.first;
        Size N = result.second;
    ...
    Real GenericRiskStatistics<S>::shortfall(Real target) const {
        QL_ENSURE(this->samples() != 0, "empty sample set");
        return this->expectationValue([target](Real x){return x<target? 1.0: 0.0;});
    }
    ...
    Real GenericRiskStatistics<S>::averageShortfall(Real target)
        const {
        std::pair<Real,Size> result =
            this->expectationValue([target](Real x){return target - x;}, [target](Real x){return x < target;});
        Real x = result.first;
        Size N = result.second;
        QL_ENSURE(N != 0, "no data below the target");
        return x;
