        ...
        if (fixingDate_ > today){
            ...
            switch (modelOfYieldCurve_) {
                ...
                case GFunctionFactory::ParallelShifts: {
                    Handle<Quote> nullMeanReversionQuote(std::make_shared<SimpleQuote>(0.0));
            ...
            vanillaOptionPricer_= std::make_shared<BlackVanillaOptionPricer>(swapRateValue_, fixingDate_, swapTenor_,
                                                                             *swaptionVolatility());
         }
    }
...
//===========================================================================//
        ...
        class Spy {
          public:
            ...
            Real value(Real x){
                ...
                Real value = f_(x);
                functionValues.emplace_back(value);
                return value;
            }
          private:
            std::function<Real (Real)> f_;
    ...
    Real NumericHaganPricer::optionletPrice(
        ...
        std::shared_ptr<ConundrumIntegrand> integrand =
            std::make_shared<ConundrumIntegrand>(vanillaOptionPricer_, rateCurve_, gFunction_,
...
//===========================================================================//
    ...
    NumericHaganPricer::ConundrumIntegrand::ConundrumIntegrand(
        const std::shared_ptr<VanillaOptionPricer>& o,
        const std::shared_ptr<YieldTermStructure>&,
        const std::shared_ptr<GFunction>& gFunction,
        Date fixingDate,
        Date paymentDate,
        Real annuity,
        Real forwardValue,
...
//===========================================================================//
    ...
    std::shared_ptr<GFunction> GFunctionFactory::newGFunctionStandard(Size q,
        ...
        return std::make_shared<GFunctionStandard>(q, delta, swapLength);
    }
...
//===========================================================================//
    ...
    GFunctionFactory::GFunctionExactYield::GFunctionExactYield(const CmsCoupon& coupon){
        ...
        accruals_.reserve(n);
        for (Size i=0; i<n; ++i) {
            std::shared_ptr<Coupon> coupon_ =
                std::dynamic_pointer_cast<Coupon>(fixedLeg[i]);
            accruals_.emplace_back(coupon_->accrualPeriod());
        }
    }
    ...
    std::shared_ptr<GFunction> GFunctionFactory::newGFunctionExactYield(const CmsCoupon& coupon) {
        return std::make_shared<GFunctionExactYield>(coupon);
    }
...
//===========================================================================//
      ...
      tmpRs_(10000000.0), accuracy_( 1.0e-14) {
        ...
        for(Size i=0; i<n; ++i) {
            std::shared_ptr<Coupon> coupon_ =
                std::dynamic_pointer_cast<Coupon>(fixedLeg[i]);
            accruals_.emplace_back(coupon_->accrualPeriod());
            const Date paymentDate(coupon_->date());
            const double swapPaymentTime(dc.yearFraction(rateCurve->referenceDate(), paymentDate));
            shapedSwapPaymentTimes_.emplace_back(shapeOfShift(swapPaymentTime));
            swapPaymentDiscounts_.emplace_back(rateCurve->discount(paymentDate));
        }
    ...
    std::shared_ptr<GFunction> GFunctionFactory::newGFunctionWithShifts(const CmsCoupon& coupon,
                                                                          const Handle<Quote>& meanReversion) {
        return std::make_shared<GFunctionWithShifts>(coupon, meanReversion);
    }
