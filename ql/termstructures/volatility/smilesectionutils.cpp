        ...
        for (Size i = 0; i < tmp.size(); i++) {
            ...
            if (tmp[i] <= QL_EPSILON ||
                ...
                if (k < section.minStrike() && !minStrikeAdded) {
                    m_.emplace_back((section.minStrike()+shift) / f_);
                    k_.emplace_back(section.minStrike());
                    minStrikeAdded = true;
                }
                if (k > section.maxStrike() && !maxStrikeAdded) {
                    m_.emplace_back((section.maxStrike()+shift)/ f_);
                    k_.emplace_back(section.maxStrike());
                    maxStrikeAdded = true;
                }
            }
        }

        c_.emplace_back(f_ + shift);

        for (Size i = 1; i < k_.size(); i++) {
            c_.emplace_back(section.optionPrice(k_[i], Option::Call, 1.0));
        }
