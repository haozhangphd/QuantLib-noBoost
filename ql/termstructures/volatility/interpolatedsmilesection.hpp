...
namespace QuantLib {
      ...
      stdDevHandles_(stdDevs.size()), vols_(stdDevs.size())
    {
        ...
        for (Size i=0; i<stdDevs.size(); ++i)
            stdDevHandles_[i] = Handle<Quote>(std::make_shared<SimpleQuote>(stdDevs[i]));
        atmLevel_ = Handle<Quote>(std::make_shared<SimpleQuote>(atmLevel));
        // check strikes!!!!!!!!!!!!!!!!!!!!
        interpolation_ = interpolator.interpolate(strikes_.begin(),
                                                  strikes_.end(),
                                                  vols_.begin());
      ...
      stdDevHandles_(stdDevs.size()), vols_(stdDevs.size())
    {
        ...
        atmLevel_ = Handle<Quote>
           (std::make_shared<SimpleQuote>(atmLevel));
        // check strikes!!!!!!!!!!!!!!!!!!!!
        interpolation_ = interpolator.interpolate(strikes_.begin(),
                                                  strikes_.end(),
                                                  vols_.begin());
