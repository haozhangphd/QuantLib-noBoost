...
namespace QuantLib {
    ...
    Real HestonBlackVolSurface::blackVarianceImpl(Time t, Real strike) const {
        return square(blackVolImpl(t, strike))*t;
    }
    ...
    Volatility HestonBlackVolSurface::blackVolImpl(Time t, Real strike) const {
        ...
        auto f = [&payoff, strike, fwd, t, df, npv] (Real r) {return blackValue(payoff.optionType(), strike, fwd, t, r, df ,npv);};

        Real temp = solver.solve(f, accuracy, guess, 0.01);
