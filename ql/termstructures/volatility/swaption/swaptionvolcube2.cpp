...
namespace QuantLib {
    ...
    SwaptionVolCube2::smileSectionImpl(const Date& optionDate,
        ...
        for (Size i=0; i<nStrikes_; ++i) {
            strikes.emplace_back(atmForward + strikeSpreads_[i]);
            stdDevs.emplace_back(exerciseTimeSqrt*(
                atmVol + volSpreadsInterpolator_[i](length, optionTime)));
        }
        Real shift = atmVol_->shift(optionTime,length);
        return std::make_shared<InterpolatedSmileSection<Linear>>(optionTime,
                                             strikes,
                                             stdDevs,
                                             atmForward,
                                             Linear(),
