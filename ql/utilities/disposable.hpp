...
namespace QuantLib {
    ...
    */
	...
	Disposable& operator=(Disposable&&) = default;
        Disposable(T&& t) {this->swap(t);}
	T& operator=(T&& t) {this->swap(t); return *t;}
        Disposable(const T&) = delete;
        Disposable<T>& operator=(const T&) = delete;
	Disposable(const Disposable<T>&) = delete;
        Disposable<T>& operator=(const Disposable<T>&) = delete;
    };
}
#endif
