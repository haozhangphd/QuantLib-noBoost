...
namespace QuantLib {
    ...
    void MarkovFunctional::updateTimes1() const {
        ...
        for (std::vector<Date>::const_iterator i = volstepdates_.begin();
            ...
            volsteptimes_.emplace_back(termStructure()->timeFromReference(*i));
    ...
    void MarkovFunctional::updateTimes2() const {
        ...
        for (std::map<Date, CalibrationPoint>::iterator k =
            ...
            times_.emplace_back(termStructure()->timeFromReference(k->first));
            modelOutputs_.expiries_.emplace_back(k->first);
            modelOutputs_.tenors_.emplace_back(k->second.tenor_);
        }
        times_.emplace_back(numeraireTime_);
    ...
    void MarkovFunctional::makeSwaptionCalibrationPoint(const Date &expiry,
        ...
        for (unsigned int k = 1; k < sched.size(); k++) {
            p.yearFractions_.emplace_back(
                swapIndexBase_->dayCounter().yearFraction(
                    k == 1 ? expiry : sched.date(k - 1), sched.date(k)));
            p.paymentDates_.emplace_back(cal.adjust(sched.date(k), bdc));
        }
    ...
    void MarkovFunctional::makeCapletCalibrationPoint(const Date &expiry) {
        ...
        p.paymentDates_.emplace_back(endDate);
        p.yearFractions_.emplace_back(
    ...
    MarkovFunctional::modelOutputs() const {
        ...
        if (modelOutputs_.dirty_) {
            ...
            for (Size i = 1; i < times_.size() - 1; i++) {
                modelOutputs_.marketZerorate_.emplace_back(
                ...
                // initial yts
                modelOutputs_.modelZerorate_.emplace_back(
            ...
            for (std::map<Date, CalibrationPoint>::iterator i =
                ...
                modelOutputs_.atm_.emplace_back(i->second.atm_);
                modelOutputs_.annuity_.emplace_back(i->second.annuity_);
                std::shared_ptr<SmileSection> sec = i->second.smileSection_;
                ...
                for (Size j = 0; j < money.size(); j++) {
                    strikes.emplace_back(money[j] * (i->second.atm_ + shift) -
                                      shift);
                    try {
                        marketRawCall.emplace_back(rawSec->optionPrice(
                            strikes[j], Option::Call, i->second.annuity_));
                        marketRawPut.emplace_back(rawSec->optionPrice(
                        ...
                        marketRawCall.emplace_back(0.0);
                        marketRawPut.emplace_back(0.0);
                    }
                    marketCall.emplace_back(sec->optionPrice(
                        strikes[j], Option::Call, i->second.annuity_));
                    marketPut.emplace_back(sec->optionPrice(
                        strikes[j], Option::Put, i->second.annuity_));
                    modelCall.emplace_back(
                        i->second.isCaplet_
                    ...
                    modelPut.emplace_back(
                        i->second.isCaplet_
                            ...
                            : swaptionPriceInternal(Option::Put, i->first,
                                               i->second.tenor_, strikes[j],
                                               Null<Date>(), 0.0, true));
                    marketVega.emplace_back(
                        sec->vega(strikes[j], i->second.annuity_));
                }
                modelOutputs_.smileStrikes_.emplace_back(strikes);
                modelOutputs_.marketCallPremium_.emplace_back(marketCall);
                modelOutputs_.marketPutPremium_.emplace_back(marketPut);
                modelOutputs_.modelCallPremium_.emplace_back(modelCall);
                modelOutputs_.modelPutPremium_.emplace_back(modelPut);
                modelOutputs_.marketVega_.emplace_back(marketVega);
                modelOutputs_.marketRawCallPremium_.emplace_back(marketRawCall);
                modelOutputs_.marketRawPutPremium_.emplace_back(marketRawPut);
            }
