...
namespace QuantLib {
    ...
    MultiStepPeriodCapletSwaptions::MultiStepPeriodCapletSwaptions(const std::vector<Time>& rateTimes,
                                     ...
                                     const std::vector<std::shared_ptr<StrikedTypePayoff> >& forwardPayOffs,
                                     const std::vector<std::shared_ptr<StrikedTypePayoff> >& swapPayOffs,
                                     Size period,
                                     Size offset)
    : MultiProductMultiStep(rateTimes),
      ...
      offset_(offset)
      {
        ...
        for (Size i=0; i < swaptionPaymentTimes_.size(); ++i)
            paymentTimes_.emplace_back(swaptionPaymentTimes_[i]);
