...
namespace QuantLib {
            ...
            for (Size r=volStructure->evolution().firstAliveRate()[s]; r < rates; ++r)
            {
                if (factorwiseBumping)
                {
                    for (Size f=0; f < factors; ++f)
                    {
                        ...
                        allBumps_.emplace_back(thisCluster);
    ...
    VegaBumpCollection::VegaBumpCollection(const std::vector<VegaBumpCluster>& allBumps,  const std::shared_ptr<MarketModel>& volStructure)
