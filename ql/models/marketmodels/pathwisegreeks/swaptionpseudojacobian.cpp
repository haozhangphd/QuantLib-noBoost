...
namespace QuantLib
{
    ...
    SwaptionPseudoDerivative::SwaptionPseudoDerivative(std::shared_ptr<MarketModel> inputModel,
        Size startIndex,
        Size endIndex)
    {
        ...
        while (index < stopIndex)
        {
            ...
            varianceDerivatives_.emplace_back(thisDerivative);

            for ( Size rate=startIndex; rate<endIndex; ++rate)
                for (Size f =0; f < factors; ++f)
                    thisDerivative[rate][f] *= scale;

            volatilityDerivatives_.emplace_back(thisDerivative);     
        ...
        {
            varianceDerivatives_.emplace_back(nullDerivative);
            volatilityDerivatives_.emplace_back(nullDerivative);
    ...
    CapPseudoDerivative::CapPseudoDerivative(std::shared_ptr<MarketModel> inputModel,
        Real strike,
        Size startIndex,
        Size endIndex, Real firstDF) : firstDF_(firstDF)
    {
        ...
        for (Size step =0; step < inputModel->evolution().numberOfSteps(); ++step)
        {
            ...
            priceDerivatives_.emplace_back(thisDerivative);
