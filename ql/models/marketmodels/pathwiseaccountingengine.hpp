...
namespace QuantLib {
    ...
    {
      public:
        PathwiseAccountingEngine(const std::shared_ptr<LogNormalFwdRateEuler>& evolver, // method relies heavily on LMM Euler
   ...
   //! Engine collecting cash flows along a market-model simulation for doing pathwise computation of Deltas and vegas
    ...
    {
      public:
        PathwiseVegasAccountingEngine(const std::shared_ptr<LogNormalFwdRateEuler>& evolver, // method relies heavily on LMM Euler
                         const Clone<MarketModelPathwiseMultiProduct>& product,
                         const std::shared_ptr<MarketModel>& pseudoRootStructure, // we need pseudo-roots and displacements
   ...
   //! Engine collecting cash flows along a market-model simulation for doing pathwise computation of Deltas and vegas
    ...
    {
      public:
        PathwiseVegasOuterAccountingEngine(const std::shared_ptr<LogNormalFwdRateEuler>& evolver, // method relies heavily on LMM Euler
                         const Clone<MarketModelPathwiseMultiProduct>& product,
                         const std::shared_ptr<MarketModel>& pseudoRootStructure, // we need pseudo-roots and displacements
                         const std::vector<std::vector<Matrix> >& VegaBumps, 
                         Real initialNumeraireValue);
