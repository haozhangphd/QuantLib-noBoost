...
namespace QuantLib
{
        ...
        noSmallRates_(timesForSmallRates.size()-1)
    {
        ...
        for (Size i=0; i < noBigRates_; ++i)
            originalVariances_[i] = std::make_shared<PiecewiseConstantAbcdVariance>(originalVariances[i]);
    ...
    void VolatilityInterpolationSpecifierabcd::recompute()
    {
...

        {
            ...
            for (Size i=0; i < offset_; ++i)
                interpolatedVariances_[i] = std::make_shared<PiecewiseConstantAbcdVariance>(a,b,c,d,i,timesForSmallRates_);
        }
        ...
        for (Size j=0; j < noBigRates_-1; ++j)
        {
            ...
            for (Size i=0; i < period_; ++i)
                interpolatedVariances_[i+j*period_+offset_] =  std::make_shared<PiecewiseConstantAbcdVariance>(a,b,c,d,i+j*period_,timesForSmallRates_);
...

       {
            ...
            // very last rate is special as we must match the caplet vol
             ...
             b*=scale;
             d*=scale;
             interpolatedVariances_[noSmallRates_-1] = std::make_shared<PiecewiseConstantAbcdVariance>(a,b,c,d,noSmallRates_-1,timesForSmallRates_);
