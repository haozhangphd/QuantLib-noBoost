...
namespace QuantLib {
    ...
    FloatFloatSwap::FloatFloatSwap(
        ...
        const std::shared_ptr<InterestRateIndex> &index1,
        const DayCounter &dayCount1, const Schedule &schedule2,
        const std::shared_ptr<InterestRateIndex> &index2,
    ...
    void FloatFloatSwap::init(
        ...
        if (intermediateCapitalExchange_) {
            for (Size i = 0; i < legs_[0].size() - 1; i++) {
                ...
                if (!close(cap, 0.0)) {
                    std::vector<std::shared_ptr<CashFlow> >::iterator it1 =
                        legs_[0].begin();
                    std::advance(it1, i + 1);
                    legs_[0].insert(
                        it1, std::make_shared<Redemption>(cap, legs_[0][i]->date()));
            ...
            for (Size i = 0; i < legs_[1].size() - 1; i++) {
                ...
                if (!close(cap, 0.0)) {
                    ...
                    std::advance(it1, i + 1);
                    legs_[1].insert(
                        it1, std::make_shared<Redemption>(cap, legs_[1][i]->date()));
        ...
        if (finalCapitalExchange_) {
            legs_[0].emplace_back(std::make_shared<Redemption>(nominal1_.back(), legs_[0].back()->date()));
            nominal1_.emplace_back(nominal1_.back());
            legs_[1].emplace_back(std::make_shared<Redemption>(nominal2_.back(), legs_[1].back()->date()));
            nominal2_.emplace_back(nominal2_.back());
        }
