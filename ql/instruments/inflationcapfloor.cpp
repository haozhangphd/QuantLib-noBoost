...
namespace QuantLib {
    ...
    capRates_(capRates), floorRates_(floorRates) {
        if (type_ == Cap || type_ == Collar) {
            ...
            while (capRates_.size() < yoyLeg_.size())
                capRates_.emplace_back(capRates_.back());
        }
        if (type_ == Floor || type_ == Collar) {
            ...
            while (floorRates_.size() < yoyLeg_.size())
                floorRates_.emplace_back(floorRates_.back());
        }
    ...
    shared_ptr<YoYInflationCapFloor> YoYInflationCapFloor::optionlet(const Size i) const {
        ...
        if (type() == Cap || type() == Collar)
            cap.emplace_back(capRates()[i]);
        if (type() == Floor || type() == Collar)
            floor.emplace_back(floorRates()[i]);

        return std::make_shared<YoYInflationCapFloor>(type(), cf, cap, floor);
    }
