...
namespace QuantLib {
    ...
    : settlementDays_(settlementDays), calendar_(calendar),
      ...
      issueDate_(issueDate) {
        ...
        if (!cashflows.empty()) {
            ...
            redemptions_.emplace_back(cashflows.back());
        }
    ...
    void Bond::setSingleRedemption(Real notional,
        ...
        cashflows_.emplace_back(redemption);
        redemptions_.emplace_back(redemption);
    }
    ...
    void Bond::calculateNotionalsFromCashflows() {
        ...
        notionalSchedule_.emplace_back(Date());
        for (Size i=0; i<cashflows_.size(); ++i) {
            shared_ptr<Coupon> coupon =
                std::dynamic_pointer_cast<Coupon>(cashflows_[i]);
            if (!coupon)
                continue;
            ...
            if (notionals_.empty()) {
                notionals_.emplace_back(coupon->nominal());
                ...
                // the previous one...
                notionalSchedule_.emplace_back(lastPaymentDate);
